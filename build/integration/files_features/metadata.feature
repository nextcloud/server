...
Feature: metadata
  ...
  Scenario: Deleting metadata works
    ...
    When User "user0" uploads file with content "AAA" to "/test.txt"
    And User "user0" sets the "metadata-files-live-photo" prop with value "metadata-value" on "/test.txt"
    And User "user0" sets the "metadata-files-live-photo" prop with value "null" on "/test.txt"
    Then User "user0" should see the prop "metadata-files-live-photo" equal to "null" for file "/test.txt"
