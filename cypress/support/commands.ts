		...
		.then(async () => {
			try {
				...
				const response = await axios({
					...
					// MKCOL answers 405 when the collection already exists. A
					// retry re-runs the test body but not the data it created,
					// so every attempt after the first would fail on set-up.
				...
				throw new Error(`Unable to create directory ${target}${status ? ` (status ${status})` : ''}`, { cause })
			}
		})
})
