...
describe('Files: Move or copy files', { testIsolation: true }, () => {
	...
	context('escaping file and folder names', () => {
		it('Can handle files with special characters', () => {
			...
			// click copy
			cy.get(`[data-filename="${CSS.escape('can\'t say')}"]`).should('be.visible').click()
			cy.contains('button', 'Copy to can\'t say').should('be.visible').click()

			cy.wait('@copyFile')
			getRowForFile('can\'t say').should('be.visible').find('[data-cy-files-list-row-name-link]').click()
		...
		it('does not incorrectly sanitize file names', () => {
			...
			// click copy
			cy.get(`[data-filename="${CSS.escape('<a href="#">foo')}"]`).should('be.visible').click()
			cy.contains('button', 'Copy to <a href="#">foo').should('be.visible').click()

			cy.wait('@copyFile')

			// Navigate to folder
			getRowForFile('<a href="#">foo').should('be.visible').find('[data-cy-files-list-row-name-link]').click()
