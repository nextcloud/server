...
class Movie extends ProviderV2 {
	...
	public function getThumbnail(File $file, int $maxX, int $maxY): ?IImage {
		...
		// If metadata indicates this is a test, force attempts to 0
		// to ensure stills for test and reference images come from the same
		// timestamp
		if ($file->getMetaData()['istest']) {
			$timeAttempts = [0];
		}
		...
		if ($this->useTempFile($file)) {
			if ($file->getStorage()->isLocal()) {
				// File is local, also attempt the entire file
				$sizeAttempts[] = null;
			}
		} else {
			// Size is irrelevant, get entire file
			$sizeAttempts = [null];
		}
