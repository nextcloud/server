...
/**
 * Creates a JPG preview of a HEIF file using ImageMagick via the PECL
 * extension.
 *
 * HEIF is the container; what is inside it depends on the codec. HEIC holds
 * h265 and AVIF holds AV1, and ImageMagick reads both through libheif, so
 * the two providers differ only in which format they claim and which one
 * they ask for by name.
 *
 * @package OC\Preview
 */
abstract class Heif extends ProviderV2 {
	/**
	 * The ImageMagick format this provider reads, lowercase, used to tell
	 * it what the file is rather than leaving it to guess.
	 */
	abstract protected function formatHint(): string;

	/**
	 * The name to ask ImageMagick whether it was built with, which is the
	 * codec's rather than the container's: a build may carry one and not
	 * the other.
	 */
	abstract protected function queryFormat(): string;
	...
	public function isAvailable(FileInfo $file): bool {
		return in_array($this->queryFormat(), \Imagick::queryFormats($this->queryFormat()), true);
	}
	...
	public function getThumbnail(File $file, int $maxX, int $maxY): ?IImage {
		...
		// Creates \Imagick object from the file
		try {
	...
	private function getResizedPreview($tmpPath, $maxX, $maxY) {
		...
		// Some files just contain (or at least are identified as) other formats
		// like JPEG. We just need to check if the image is safe to process.
		$bp->pingImage($this->formatHint() . ':' . $tmpPath . '[0]');
		...
		$bp->readImage($this->formatHint() . ':' . $tmpPath . '[0]');
