...
Per-user synthesis always covers every affected user, so listener-driven
state (encryption keys, share targets, mount cache, ...) stays consistent
regardless of hierarchy size. A soft warning threshold
(`Manager::$perUserEventWarnThreshold`, default `500`) only controls when
a "this may take a moment" warning is logged; events are never dropped.
Bulk mutations on huge groups can therefore take noticeable time —
prefer running them via `occ` during off-hours.
