...
class AuthorizedGroupMapper extends QBMapper {
	...
	public function findAllClassesForUser(IUser $user): array {
		...
		// Admin delegation must honor nested-group edges: a user who is a
		// member of a sub-group of an authorized group should inherit the
		// delegation.
		$groupIds = $groupManager->getUserEffectiveGroupIds($user);
		if (count($groupIds) === 0) {
			return [];
		}
