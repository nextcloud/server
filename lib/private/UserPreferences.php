...
class UserPreferences implements IUserPreferences {
	 ...
	 * @return list<string>
	 */
	private function searchUsersByTypedValue(string $app, string $key, string|array $value, bool $caseInsensitive = false, bool $withinNotIndexedField = false): array {
		$this->assertParams('', $app, $key, allowEmptyUser: true);
		...
		// TODO: remove $configValueColumn once $withinNotIndexedField is fully deprecated
		...
		if (is_array($value)) {
			...
			// TODO: remove this condition once $withinNotIndexedField is fully deprecated
			if ($withinNotIndexedField) {
				// search within indexed OR configvalue
				$where = $qb->expr()->orX(
					$where,
