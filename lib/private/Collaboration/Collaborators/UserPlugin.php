	...
	public function search($search, $limit, $offset, ISearchResult $searchResult): bool {
		...
		if ($shareeEnumerationFullMatch && $search !== '') {
			...
			if ($shareeEnumerationFullMatchEmail) {
				// TODO: Defer search to the manager/backends
				$this->userManager->callForAllUsers(function (IUser $user) use (&$users, $lowerSearch): void {
					$email = $user->getSystemEMailAddress();
					if ($email !== null && mb_strtolower($email) === $lowerSearch) {
						$users[$user->getUID()] = ['exact', $user];
					}
				});
			}
