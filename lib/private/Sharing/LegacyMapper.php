...
final readonly class LegacyMapper {
	...
	public function createLegacyMapping(string $id, string $legacyProvider, int $legacyId, DateTimeImmutable $lastUpdated, string $recipientSecret, string $recipientValue): LegacyMapping {
		...
		$qb
			...
			->values([
				...
				'recipient_secret' => $qb->createNamedParameter($recipientSecret),
				'recipient_value' => $qb->createNamedParameter($recipientValue),
			])
			->executeStatement();
	...
	public function getLegacyMappingByLegacyProviderAndId(string $legacyProvider, int $legacyId): ?LegacyMapping {
		$qb = $this->dbConnection->getQueryBuilder();
		$result = $qb
			->select('id', 'last_updated', 'recipient_secret', 'recipient_value')
	...
	public function getLegacyMappings(string $id): array {
		...
		$qb = $this->dbConnection->getQueryBuilder();
		$result = $qb
			->select('id', 'legacy_provider', 'legacy_id', 'last_updated', 'recipient_secret', 'recipient_value')
			->from('sharing_share_legacy_mapping')
	...
	public function updateLegacyMapping(LegacyMapping $legacyMapping): void {
		...
		$qb
			...
			->set('recipient_secret', $qb->createNamedParameter($legacyMapping->recipientSecret))
			->set('recipient_value', $qb->createNamedParameter($legacyMapping->recipientValue))
