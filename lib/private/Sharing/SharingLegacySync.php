	...
	private function syncLegacySharesToShare(string $id, array $legacyMappings): void {
		...
		foreach ($legacyShares as $legacyShare) {
			if ($legacyShare->getShareType() === IShare::TYPE_USER) {
				...
				$this->sourceNodeTargetManager->setTarget($legacyShare->getSharedWith(), $owner, $legacyShare->getNodeId(), $legacyShare->getTarget());
				...
				foreach ($rows as $row) {
					...
					$this->sourceNodeTargetManager->setTarget($row['share_with'], $owner, $legacyShare->getNodeId(), $row['file_target']);
				}
			}
		}
	}
		...
		foreach ($share->recipients as $recipient) {
			...
			foreach ($share->sources as $source) {
				...
				if ($recipient->class === UserShareRecipientType::class && $recipient->instance === null) {
					$userStatus = $this->backend->getUserStatuses($share->id, [$recipient->value])[$recipient->value] ?? null;
					$target = $this->sourceNodeTargetManager->getTarget($recipient->value, $share->owner, (int)$source->value) ?? $this->sourceNodeTargetManager->createDefaultTarget($recipient->value, $share->owner, (int)$source->value);
				}
				...
				$legacyShare->setStatus($this->userStatusToLegacyUserStatus($userStatus ?? ShareUserStatus::Pending));
				...
				if ($recipient->class === GroupShareRecipientType::class) {
					...
					$userIdsByTarget = [];
					foreach ($this->sourceNodeTargetManager->getTargets($userIds, $share->owner, (int)$source->value) as $userId => $target) {
