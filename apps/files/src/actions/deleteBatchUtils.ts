...
import { isBulkDeleteItem, runBulkDelete } from '../services/bulkDelete.ts'
...
export function deleteNodesInBatches(nodes: INode[], view: IView, queue: PQueue): Promise<boolean[]> | null {
   ...
   try {
      ...
      // Invalid URL or percent encoding: do not manufacture a different path.
      return null
   }
   ...
   return runBulkDelete(files, {
      ...
      concurrency: 5,
      async request(batch) {
         // Reuse the existing queue, rather than multiplying its concurrency.
         return queue.add(async () => {
            var response = await axios.post(`${davRoot}/bulk-delete`, { files: batch }, {
      ...
      onError(error) {
         logger.error('Bulk deletion failed; refresh the file list before retrying', { error })
      },
   })
}
