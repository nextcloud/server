...
<template>
	<div v-click-outside="hideDelete" class="check" @click="showDelete">
		...
		<NcSelect v-model="currentOperator"
			...
			@input="updateCheck" />
		<component :is="currentWebComponent"
			v-if="currentWebComponent"
			ref="checkComponent"
			:disabled="!currentOption"
			:check="check"
...
export default {
	...
	computed: {
		...
		currentWebComponent() {
			...
			return this.checks[this.check.class].webComponent
		},
		currentComponent() {
			if (!this.currentOption) { return [] }
	...
	mounted() {
		...
		if (this.currentWebComponent) {
			...
			console.warn('Developer warning: `CheckPlugin.options` is deprecated. Use `CheckPlugin.webComponent` instead.')
		}
