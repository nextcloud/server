...
<template>
	<NcDialog
		...
		<form
			...
			<NcNoteCard
				v-if="hasNonAsciiUsername"
				type="warning">
				{{ t('settings', 'The account name contains non-ASCII characters. This requires the "allow_unicode_usernames" server option and may break external storage, LDAP, federation, clients or apps that expect ASCII-only account names.') }}
			</NcNoteCard>
		</form>
...
// Whether the entered account name contains non-ASCII characters.
// Used to warn about possible breakage, this is only accepted by the
// server if 'allow_unicode_usernames' is enabled.
const hasNonAsciiUsername = computed(() => /[^\x20-\x7E]/.test(props.newUser.username ?? ''))
