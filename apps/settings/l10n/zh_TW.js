OC.L10N.register(
    ...
    "Scheduling objects table size" : "調度物件表格大小",
    "You have more than 500 000 rows in the scheduling objects table. Please run the expensive repair jobs via occ maintenance:repair --include-expensive" : "調度物件表格中超過五十萬列。請透過 occ maintenance:repair --include-expensive 執行昂貴的修復作業",
    "Scheduling objects table size is within acceptable range." : "調度物件表格大小在可接受的範圍內。",
    ...
    "Allow account name autocompletion to users within the same groups and limit system address books to users in the same groups" : "允許同一個群組內的使用者自動完成帳號名稱，並將系統通訊錄限制為同一個群組內的使用者",
    "Allow account name autocompletion to users based on phone number integration" : "允許以電話號碼整合為基礎的帳號名稱自動完成",
