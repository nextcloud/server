OC.L10N.register(
    ...
    "Two-factor authentication can be enforced for all accounts and specific groups. If they do not have a two-factor provider configured, they will be unable to log into the system." : "可以对所有帐户和特定组强制执行两因素身份验证。 如果他们没有配置双因素提供商，他们将无法登录系统。",
    ...
    "You do not have permissions to see the details of this account" : "您无权查看此帐号的详细信息",
    "Set new email address" : "设置新邮箱地址",
    "Add account to group" : "将帐户添加到组",
    "Set account as admin for" : "将帐号设置为管理员",
    ...
    "Toggle account actions menu" : "切换帐号操作菜单",
    "Done" : "完成",
    "Edit" : "编辑",
    ...
    "To allow this check to run you have to make sure that your webserver can connect to itself. Therefore it must be able to resolve and connect to at least one its `trusted_domains` or the `overwrite.cli.url`." : "要允许此检查运行，您必须确保您的网络服务器可以连接到自身。 因此，它必须能够解析并连接到至少一个 `trusted_domains` 或 `overwrite.cli.url`。",
