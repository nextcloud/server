OC.L10N.register(
    ...
    "_You revoked %n other session_::_You revoked %n other sessions_" : ["%n 件の他のセッションを解除しました"],
    ...
    "Availability" : "応対可能日時",
    "Calendar" : "カレンダー",
    "Language & locale" : "言語とロケール",
    "Profile & contact" : "プロフィールと連絡先",
    ...
    "for Argon2 for password hashing and Ed25519 signature verification for RFC 9421 http message signatures" : "パスワードハッシュにはArgon2を、RFC 9421のHTTPメッセージ署名の検証にはEd25519を使用する",
    ...
    "MariaDB version 10.6 detected, this version is end-of-life and only supported as part of Ubuntu 22.04. MariaDB >=%1$s and <=%2$s is suggested for best performance, stability and functionality with this version of Nextcloud." : "MariaDBバージョン10.6が検出されましたが、このバージョンはサポートが終了しており、Ubuntu 22.04の一部としてのみサポートされています。このバージョンのNextcloudで最高のパフォーマンス、安定性、機能性を実現するには、MariaDB >=%1$s および <=%2$sを推奨します。",
    ...
    "_No scheduled tasks in the last %n day._::_No scheduled tasks in the last %n days._" : ["過去%n日間にスケジュールされたタスクはありません。"],
    "_Most tasks were successful in the last %n day._::_Most tasks were successful in the last %n days._" : ["ほとんどのタスクは過去%n日間に成功しました。"],
    ...
    "_The Task Processing worker has run in the last %n minute._::_The Task Processing worker has run in the last %n minutes._" : ["タスク処理ワーカーは、過去%n分間稼働しています。"],
    ...
    "Hide legacy provider settings" : "レガシープロバイダーの設定を非表示にする",
    "Show legacy provider settings" : "レガシープロバイダーの設定を表示する",
    "Machine translation (Legacy)" : "機械翻訳（旧バージョン）",
    ...
    "This API is deprecated. Install apps that provide Unified task processing to get the best experience." : "この API は非推奨となっています。最適な体験を得るには、統合タスク処理機能を備えたアプリをインストールしてください。",
    "Learn more about AI providers" : "AIプロバイダーについて詳しく知る",
    "Image generation (Legacy)" : "画像生成（旧バージョン）",
    ...
    "Text processing (Legacy)" : "テキスト処理（旧バージョン）",
    ...
    "Exclude some groups (block list)" : "一部のグループを除外する（ブロックリスト）",
    "Allow some groups only (allow list)" : "特定のグループのみ許可する（許可リスト）",
    "Groups allowed to share" : "共有を許可されたグループ",
    "Groups excluded from sharing" : "共有から除外されたグループ",
    "Only members of at least one of these groups will be able to initiate shares." : "これらのグループのうち、少なくとも1つに所属しているメンバーのみが、共有を開始することができます。",
    "Others will be blocked, but are still able to receive shares." : "その他のユーザーはブロックされますが、共有を受け取ることは可能です。",
    "A member of at least one of these groups will not be able to initiate new shares, but will still be able to receive shares." : "これらのグループのうち少なくとも1つに属するメンバーは、新しい共有を発行することはできませんが、共有を受け取ることは引き続き可能です。",
    ...
    "Revoke and cancel pending wipe?" : "保留中のワイプを取り消してキャンセルしますか?",
    "Only continue if you no longer need the device to be wiped." : "デバイスをワイプする必要がなくなった場合にのみ続行します。",
    "Revoke and cancel wipe" : "ワイプの取り消しとキャンセル",
    "Revoke app password?" : "アプリ用パスワードを取り消しますか？",
    "The app or device will lose access on its next sync. This cannot be undone." : "アプリまたはデバイスは、次の同期時にアクセスできなくなります。これは元に戻せません。",
    "Remote wipe has not started yet." : "リモートワイプがまだ開始されていません。",
    "Revoking now cancels the wipe. The device keeps its synced data." : "ここで取り消すと、ワイプがキャンセルされます。デバイスは同期されたデータを保持します。",
    "Device" : "端末",
    "Last activity" : "最後のアクティビティ",
    "Actions" : "アクション",
    "Revoke all others" : "他の全てを取り消す",
    "Revoke all other sessions?" : "他の全てのセッションを解除しますか？",
    "Sync clients and connected services have to sign in again. This cannot be undone." : "同期クライアントおよび接続済みのサービスでは、再度サインインする必要があります。この操作は元に戻せません。",
    ...
    "Revoke all other sessions" : "他のすべてのセッションを解除する",
    "Signs out every device and app except this one." : "これ以外のすべてのデバイスとアプリからサインアウトします。",
    ...
    "Choose from Nextcloud Files" : "Nextcloud Filesから選択",
    ...
    "Primary email address" : "メインのメールアドレス",
    "Additional email address {index}" : "その他のメールアドレス {index}",
    "{label} (confirmation sent)" : "{label}（確認メールを送信済み）",
    ...
    "Nextcloud profile" : "Nextcloudのプロフィール",
    "View full profile" : "プロフィール全体を表示",
    "she/her" : "彼女",
    "he/him" : "彼",
    "they/them" : "彼ら/彼女ら",
    ...
    "Change visibility and scope of {property}" : "{property} の可視性とスコープを変更する",
    "Unable to update visibility of {property}" : "{property}の表示設定を更新できませんでした",
    "Unable to update scope of {property}" : "{property} のスコープを更新できませんでした",
    "Visibility & Scope" : "可視性とスコープ",
    ...
    "Only letters, numbers, spaces, and _.@-' are allowed" : "使用できるのは、英字、数字、スペース、および _.@-' のみです",
    "Display name" : "表示名",
    ...
    "Organization" : "共同作業",
    "Contact" : "連絡先",
    "Notifications and password reset requests will be sent to your primary email" : "通知やパスワード再設定のリクエストは、ご登録のメインメールアドレス宛てに送信されます",
    ...
    "Search accounts and groups…" : "アカウントとグループを検索…",
    ...
    "_Revoked %n other session_::_Revoked %n other sessions_" : ["%n件の他のセッションを解除しました"],
    "Could not revoke the other sessions" : "他のセッションを取り消すことができませんでした",
