...
class ShareInfoController extends ApiController {
	 ...
	 * @return JSONResponse<Http::STATUS_OK, FilesSharingShareInfo, array{}>|JSONResponse<Http::STATUS_FORBIDDEN|Http::STATUS_NOT_FOUND, \stdClass, array{}>
	 *
	...
	public function info(string $t, ?string $password = null, ?string $dir = null, int $depth = -1): JSONResponse {
		try {
			...
			$response = new JSONResponse(new \stdClass(), Http::STATUS_NOT_FOUND);
			$response->throttle(['token' => $t]);
			return $response;
		}

		if ($share->getPassword() && !$this->shareManager->checkPassword($share, $password)) {
			$response = new JSONResponse(new \stdClass(), Http::STATUS_FORBIDDEN);
			$response->throttle(['token' => $t]);
			return $response;
		}
