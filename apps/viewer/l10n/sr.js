OC.L10N.register(
    "viewer",
    {
    "Viewer" : "Прегледач",
    "Simple file viewer with slideshow for media" : "Једноставни прегледач фајлова са слајд-шоу подршком за медијске фајлове",
    "Show your latest holiday photos and videos like in the movies, show a glimpse of your latest novel directly from your Nextcloud, choose the best GIF of your collection thanks to the direct preview of your favorites files and many more!" : "Прикажите ваше најновије фотографије и видео снимке са одмора као у филмовима, прикажите брзи поглед у ваш најновији роман директно из свог Nextcloud, изаберите најбољи GIF из своје колекције захваљујући директном прегледу својих омиљених фајлова и још много, много тога!",
    "Your browser does not support audio." : "Ваш веб читач не подржава аудио.",
    "Error loading {name}" : "Грешка при учитавању {name}",
    "You are about to overwrite the original file. Are you sure you want to continue?" : "Управо ћете преписати оригинални фајл. Да ли заиста желите да наставите?",
    "A file with this name already exists. Do you want to overwrite it?" : "Фајл са овим именом већ постоји. Желите ли да га препишете?",
    "Confirm overwrite" : "Потврда преписивања",
    "Cancel" : "Поништи",
    "Overwrite" : "Препиши",
    "An error occurred while trying to confirm the file overwrite." : "Дошло је до грешке приликом покушаја потврде преписивања фајла.",
    "Image saved" : "Слика сачувана.",
    "Error saving image" : "Грешка прикиком чувања слике",
    "Play the live photo" : "Пусти фотографију уживо",
    "LIVE" : "УЖИВО",
    "Your browser does not support videos." : "Ваш веб читач не подржава видео записе.",
    "This file no longer exists" : "Фајл више не постоји",
    "There is no plugin available to display this file type" : "Нема додатка којим може да се прикаже овај тип фајла",
    "Edit" : "Измени",
    "Exit full screen" : "Напусти пун екран",
    "Full screen" : "Пун екран",
    "Open sidebar" : "Отвори бочну траку",
    "Download" : "Преузми",
    "Delete" : "Избриши",
    "View" : "Погледај",
    "Name" : "Име",
    "Save" : "Сачувај",
    "Save as" : "Сачувај као",
    "Back" : "Назад",
    "Loading …" : "Учитавање…",
    "Reset" : "Ресетуј",
    "All changes will be lost." : "Изгубиће се све измене.",
    "Are you sure you want to continue?" : "Да ли сте сигурни да желите да наставите?",
    "Apply" : "Примени",
    "Warning" : "Упозорење",
    "Confirm" : "Потврди",
    "Discard changes" : "Одбаци измене",
    "Undo" : "Опозови",
    "Redo" : "Понови",
    "Show original image" : "Прикажи оригиналну слику",
    "Zoom in" : "Увеличај",
    "Zoom out" : "Умањи приказ",
    "Toggle zoom menu" : "Укључи/искључи мени увећања",
    "Adjust" : "Подеси",
    "Fine-tune" : "Фино подешавање",
    "Filters" : "Филтери",
    "Watermark" : "Водени жиг",
    "Draw" : "Цртање",
    "Resize" : "Промена величине",
    "Image name" : "Име слике",
    "Invalid image." : "Неисправна слика.",
    "Error while uploading the image." : "Грешка приликом отпремања слике.",
    "are not images" : "нису слике",
    "is not an image" : "није слика",
    "to be uploaded" : "треба да се отпреми",
    "Crop" : "Опсецање",
    "Original" : "Оригинал",
    "Custom" : "Прилагођено",
    "Square" : "Квадрат",
    "Landscape" : "Положено",
    "Portrait" : "Усправно",
    "Ellipse" : "Елипса",
    "Classic TV" : "Класични ТВ",
    "CinemaScope" : "Синемаскоп",
    "Arrow" : "Стрелица",
    "Blur" : "Замућење",
    "Brightness" : "Осветљај",
    "Contrast" : "Контраст",
    "Un-flip X" : "Поништи обртање око X",
    "Flip X" : "Обрни око X",
    "Un-flip Y" : "Поништи обртање око Y",
    "Flip Y" : "Обрни око Y",
    "HSV" : "HSV",
    "Hue" : "Hue (Нијанса)",
    "Saturation" : "Saturation (Засићење)",
    "Value" : "Вредност",
    "Image" : "Слика",
    "Importing …" : "Увоз ...",
    "+ Add image" : "+ Додај слику",
    "Upload image" : "Отпреми слику",
    "From gallery" : "Из галерије",
    "Line" : "Линија",
    "Pen" : "Оловка",
    "Polygon" : "Полигон",
    "Sides" : "Странице",
    "Rectangle" : "Правоугаоник",
    "Corner Radius" : "Полупречник угла",
    "Width in pixels" : "Ширина у пикселима",
    "Height in pixels" : "Висина у пикселима",
    "Toggle ratio lock" : "Укључи/искључи закључавање односа страница",
    "Reset to original image size" : "Ресетуј на оригиналну величину слике",
    "Rotate" : "Ротирање",
    "Text" : "Текст",
    "Text spacing" : "Проред текста",
    "Text alignment" : "Поравнање текста",
    "Font family" : "Фамилија фонта",
    "Size" : "величина",
    "Letter spacing" : "Размак слова",
    "Line height" : "Висина линије",
    "Warmth" : "Топлина",
    "+ Add watermark" : "+ Додај водени жиг",
    "+ Add text watermark" : "+ Додај текстуални водени жиг",
    "Choose watermark type" : "Изаберите тип воденог жига",
    "Upload watermark" : "Отпреми водени жи",
    "Add as text" : "Додај као текст",
    "Padding" : "Испуна",
    "Paddings" : "Испуне",
    "Shadow" : "Сенка",
    "Horizontal" : "Хоризонтална",
    "Vertical" : "Вертикална",
    "Opacity" : "Непрозирност",
    "Transparency" : "Провидност",
    "Position" : "Позиција",
    "Stroke" : "Потез",
    "Save image as" : "Сачувај слику као",
    "Extension" : "Екстензија",
    "Format" : "Формат",
    "Name is required." : "Потребно је име.",
    "Quality" : "Квалитет",
    "Saved image size (width x height)" : "Величина сачуване слике (ширина x висина)",
    "Note that the selected crop area is lower than the applied resize which might cause quality decrease" : "Приметите да је величина изабране површине опсецања мања од примењене промене величине, а то може да умањи квалитет",
    "Actual size (100%)" : "Стварна величина (100%)",
    "Fit size" : "Уклопи величину",
    "Select image to add …" : "Изаберите слику која ће да се дода",
    "Failed to load image." : "Није успело учитавање слике.",
    "Menu" : "Мени",
    "Width" : "Ширина",
    "Height" : "Висина",
    "No preview available for this crop item" : "Није доступан преглед за ову ставку опсецања"
},
"nplurals=3; plural=(n%10==1 && n%100!=11 ? 0 : n%10>=2 && n%10<=4 && (n%100<10 || n%100>=20) ? 1 : 2);");
