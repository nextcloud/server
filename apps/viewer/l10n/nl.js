OC.L10N.register(
    "viewer",
    {
    "Viewer" : "Viewer",
    "Simple file viewer with slideshow for media" : "Eenvoudige bestand viewer met diavertoning voor media bestanden",
    "Show your latest holiday photos and videos like in the movies, show a glimpse of your latest novel directly from your Nextcloud, choose the best GIF of your collection thanks to the direct preview of your favorites files and many more!" : "Toon je laatste vakantie foto's en videos zoals in de films, toon een deel van je laatste boek direct vanuit je Nextcloud, kies de beste GIF uit jouw collectie dankzij de directe voorvertoning van je favoriete bestanden en nog veel meer!",
    "Your browser does not support audio." : "Je browser ondersteunt geen audio.",
    "Error loading {name}" : "Fout bij laden van {name}",
    "You are about to overwrite the original file. Are you sure you want to continue?" : "Je gaat het originele bestand overschrijven. Weet je zeker dat je door wilt gaan?",
    "A file with this name already exists. Do you want to overwrite it?" : "Er bestaat al een bestand met deze naam. Wil je deze overschijven?",
    "Confirm overwrite" : "Bevestig overschrijven",
    "Overwrite" : "Overschrijven",
    "Cancel" : "Annuleren",
    "An error occurred while trying to confirm the file overwrite." : "Er trad een fout op bij het toestaan van het overschrijven van het bestand",
    "Image saved" : "Afbeelding opgeslagen",
    "Error saving image" : "Opslaan van afbeelding mislukt",
    "Play the live photo" : "Speel de live foto",
    "LIVE" : "LIVE",
    "Your browser does not support videos." : "Je browser ondersteunt geen videos.",
    "This file no longer exists" : "Het bestand bestaat niet meer",
    "There is no plugin available to display this file type" : "Er is geen plug-in beschikbaar om dit bestandstype weer te geven",
    "Failed to save file before download" : "Het opslaan van het bestand voor het downloaden is mislukt",
    "Edit" : "Bewerk",
    "Exit full screen" : "Verlaat volledig scherm",
    "Full screen" : "Volledig scherm",
    "Open sidebar" : "Open zijbalk",
    "Download" : "Downloaden",
    "Delete" : "Verwijderen",
    "View" : "Bekijken",
    "Name" : "Naam",
    "Save" : "Opslaan",
    "Save as" : "Opslaan als",
    "Back" : "Terug",
    "Loading …" : "Laden …",
    "Reset" : "Herstellen",
    "All changes will be lost." : "Alle aanpassingen zullen verloren gaan.",
    "Are you sure you want to continue?" : "Weet je zeker dat je wilt doorgaan?",
    "Apply" : "Pas toe",
    "Warning" : "Waarschuwing",
    "Confirm" : "Bevestigen",
    "Discard changes" : "Negeer wijzigingen",
    "Undo" : "Ongedaan maken",
    "Redo" : "Opnieuw doen",
    "Show original image" : "Toon originele afbeelding",
    "Zoom in" : "Zoom in",
    "Zoom out" : "Uitzoomen",
    "Toggle zoom menu" : "Toon/verberg zoommenu",
    "Adjust" : "Aanpassen",
    "Fine-tune" : "Verfijnen",
    "Filters" : "Filters",
    "Watermark" : "Watermerk",
    "Draw" : "Tekenen",
    "Resize" : "Wijzig formaat",
    "Image name" : "Afbeelding naam",
    "Invalid image." : "Ongeldige afbeelding.",
    "Error while uploading the image." : "Fout bij uploaden van afbeelding.",
    "are not images" : "zijn geen afbeeldingen",
    "is not an image" : "is geen afbeelding",
    "to be uploaded" : "om te uploaden",
    "Crop" : "Bijsnijden",
    "Original" : "Origineel",
    "Custom" : "Maatwerk",
    "Square" : "Vierkant",
    "Landscape" : "Landschap",
    "Portrait" : "Portret",
    "Ellipse" : "Ovaal",
    "Classic TV" : "Klassieke TV",
    "CinemaScope" : "CinemaScope",
    "Arrow" : "Pijl",
    "Blur" : "Vervagen",
    "Brightness" : "Helderheid",
    "Contrast" : "Contrast",
    "Un-flip X" : "Ontspiegelen X",
    "Flip X" : "Spiegel X",
    "Un-flip Y" : "Spiegel Y",
    "Flip Y" : "Ontspiegel Y",
    "HSV" : "HSV",
    "Hue" : "Tint",
    "Saturation" : "Verzadiging",
    "Value" : "Waarden",
    "Image" : "Afbeelding",
    "Importing …" : "Importeren ...",
    "+ Add image" : "+ Afbeelding toevoegen",
    "Upload image" : "Upload afbeelding",
    "From gallery" : "Van gallerij",
    "Line" : "Lijn",
    "Pen" : "Pen",
    "Polygon" : "Veelhoek",
    "Sides" : "Zijdes",
    "Rectangle" : "Rechthoek",
    "Corner Radius" : "Hoekradius",
    "Width in pixels" : "Breedte in pixels",
    "Height in pixels" : "Hoogte in pixels",
    "Toggle ratio lock" : "In-/uitschakelen ratio vergrendeling",
    "Reset to original image size" : "Herstel naar oorspronkelijk afbeeldingsformaat",
    "Rotate" : "Draaien",
    "Text" : "Tekst",
    "Text spacing" : "Tekstruimte",
    "Text alignment" : "Tekstuitlijning",
    "Font family" : "Font familie",
    "Size" : "Grootte",
    "Letter spacing" : "Letterruimte",
    "Line height" : "Regelhoogte",
    "Warmth" : "Warmte",
    "+ Add watermark" : "+ Voeg watermerk toe",
    "+ Add text watermark" : "+ Voeg tekst watermerk toe",
    "Choose watermark type" : "Kies watermerk type",
    "Upload watermark" : "Upload watermerk",
    "Add as text" : "Voeg toe als tekst",
    "Padding" : "Opvulling",
    "Paddings" : "Opvullingen",
    "Shadow" : "Schaduw",
    "Horizontal" : "Horizontaal",
    "Vertical" : "Verticaal",
    "Opacity" : "Ondoorzichtigheid",
    "Transparency" : "Doorzichtigheid",
    "Position" : "Positie",
    "Stroke" : "Rand",
    "Save image as" : "Bewaar bestand als",
    "Extension" : "Extensie",
    "Format" : "Formaat",
    "Name is required." : "Naam is verplicht",
    "Quality" : "Kwaliteit",
    "Saved image size (width x height)" : "Grootte van opgeslagen afbeelding (breedte x hoogte)",
    "Note that the selected crop area is lower than the applied resize which might cause quality decrease" : "Let op dat de geselecteerde bijsnijruimte kleiner is dan de gekozen formaatswijziging wat kwaliteitsverlies kan geven",
    "Actual size (100%)" : "Ware grootte (100%)",
    "Fit size" : "Passend maken",
    "Select image to add …" : "Kies afbeelding om toe te voegen ...",
    "Failed to load image." : "Laden van afbeelding mislukt.",
    "Menu" : "Menu",
    "Width" : "Breedte",
    "Height" : "Hoogte",
    "No preview available for this crop item" : "Geen voorbeeld beschikbaar voor dit bijsnij item"
},
"nplurals=2; plural=(n != 1);");
