...
class OC_Filestorage_SFTP extends OC_Filestorage_Common {
  private $host;
	...
	public function mkdir($path) {
		return $this->client->mkdir($this->path($path));
	}

	public function rmdir($path) {
		return $this->client->delete($this->path($path), true);
	}

	public function opendir($path) {
		$list = $this->client->nlist($this->path($path));
		try {
			$id = md5('sftp:' . $path);
	...
	public function filetype($path) {
		$stat = $this->client->stat($this->path($path));
	...
	public function file_exists($path) {
		return $this->client->stat($this->path($path)) === false ? false : true;
	}
	...
	public function touch($path, $mtime=null) {
		if (!$this->file_exists($path)) {
			$this->client->put($this->path($path), '');
		}
	}
	...
	public function rename($source, $target) {
		return $this->client->rename($this->path($source), $this->path($target));
	}
