OC.L10N.register(
    ...
    "Not recommended, use it for testing only! If connection only works with this option, import the LDAP server's SSL certificate in your {instanceName} server." : "不推荐，仅用于测试！如果连接仅适用于此选项，请在您的 {instanceName} 服务器中导入 LDAP 服务器的 SSL 证书。",
    ...
    "`$home` Placeholder Field" : "`$home` 占位符字段",
    ...
    "UUID Attribute for Users" : "用户的 UUID 属性",
    "UUID Attribute for Groups" : "组的 UUID 属性",
    "Groups meeting these criteria are available in {instanceName}:" : "符合这些条件的组在 {instanceName} 中可用：",
    "Only these object classes:" : "只有这些对象类：",
    "Only from these groups:" : "仅从这些组：",
    "Edit LDAP Query" : "编辑 LDAP 查询",
    "The filter specifies which LDAP groups shall have access to the {instanceName} instance." : "筛选条件指定哪些 LDAP 组应该具有访问 {instanceName} 实例的权限。",
    "LDAP Filter:" : "LDAP 筛选：",
    "Verify settings and count the groups" : "验证设置和统计分组数",
    "User not found. Please check your login attributes and username. Effective filter (to copy-and-paste for command-line validation): {filter}" : "未找到用户。请检查您的登录属性和用户名。有效筛选条件（用于复制和粘贴以进行命令行验证）：{filter}",
    ...
    "The filter specifies which LDAP users shall have access to the {instanceName} instance." : "筛选条件指定哪些 LDAP 用户应该具有访问 {instanceName} 实例的权限。",
    "Verify settings and count users" : "验证设置和统计用户",
    "User count: {usersCount}" : "用户计数：{usersCount}",
    ...
    "Advanced" : "高级",
    "Expert" : "专家",
    "The PHP LDAP module is not installed, the backend will not work. Please ask your system administrator to install it." : "未安装 PHP LDAP 模块，后端将无法工作。请让您的系统管理员安装它。",
    "Select LDAP Config" : "选择 LDAP 配置",
    ...
    "Please renew your password" : "请更新你的密码",
