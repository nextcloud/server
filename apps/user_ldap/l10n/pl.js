OC.L10N.register(
    ...
    "Not recommended, use it for testing only! If connection only works with this option, import the LDAP server's SSL certificate in your {instanceName} server." : "Niezalecane, używaj tylko do testów! Jeśli połączenie działa tylko z tą opcją, zaimportuj certyfikat SSL serwera LDAP do swojego serwera {instanceName}.",
    ...
    "UUID Attribute for Users" : "Atrybut UUID dla użytkowników",
    "UUID Attribute for Groups" : "Atrybut UUID dla grup",
    "Groups meeting these criteria are available in {instanceName}:" : "Grupy spełniające te kryteria są dostępne w {instanceName}:",
    ...
    "The filter specifies which LDAP groups shall have access to the {instanceName} instance." : "Filtr określa, które grupy LDAP mają mieć dostęp do instancji {instanceName}",
    "LDAP Filter:" : "Filtr LDAP",
    "Verify settings and count the groups" : "Zweryfikuj ustawienia i policz grupy",
    "User not found. Please check your login attributes and username. Effective filter (to copy-and-paste for command-line validation): {filter}" : "Nie znaleziono użytkownika. Sprawdź swoje atrybuty logowania i nazwę użytkownika. Efektywny filtr (do skopiowania i wklejenia przy walidacji wiersza poleceń): {filter}",
    ...
    "The filter specifies which LDAP users shall have access to the {instanceName} instance." : "Filtr określa, którzy użytkownicy LDAP mają mieć dostęp do instancji {instanceName}",
    "Verify settings and count users" : "Sprawdź ustawienia i policz użytkowników",
    "User count: {usersCount}" : "Liczba użytkowników: {usersCount}",
    ...
    "Expert" : "Eksperckie",
    "The PHP LDAP module is not installed, the backend will not work. Please ask your system administrator to install it." : "Moduł PHP LDAP nie jest zainstalowany, backend nie będzie działał. Skontaktuj się z administratorem systemu w celu instalacji.",
    "Select LDAP Config" : "Wybierz konfigurację LDAP",
    "Create New Config" : "Utwórz nową konfigurację",
