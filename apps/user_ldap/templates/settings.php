	...
	<fieldset id="ldapSettings-1">
		<div id="ldapAdvancedAccordion">
			...
			<div>
				...
				<p><label for="ldap_change_pwd_as_user"><?php p($l->t('Do LDAP password changes as the initiator user'));?></label><span class="inlinetable"><span class="tablerow left"><input type="checkbox" id="ldap_change_pwd_as_user" name="ldap_change_pwd_as_user" value="1" data-default="<?php p($_['ldap_change_pwd_as_user_default']); ?>" title="<?php p($l->t('Change the LDAP user password as the user, who originally initiated the user password change. The user password changes initiated by the Super Administrators and Group Administrators will still be done as the LDAP service user configured in the LDAP / AD integration server settings.'));?>" /></span>
			</span><br/></p>
