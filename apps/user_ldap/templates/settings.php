...
?>
	...
	<fieldset id="ldapSettings-2">
		...
		<p class="ldapIndent"><label for="ldap_expert_force_uuid_case"><?php p($l->t('Force Case for UUIDs:'));?></label><select id="ldap_expert_force_uuid_case" name="ldap_expert_force_uuid_case" data-default="<?php p($_['ldap_expert_force_uuid_case_default']); ?>" ><option value="accept"<?php if (isset($_['ldap_expert_force_uuid_case']) && ($_['ldap_expert_force_uuid_case'] === 'accept')) {
					p(' selected');
				} ?>><?php p($l->t('Accept UUID case from LDAP'));?></option><option value="lowercase"<?php if (isset($_['ldap_expert_force_uuid_case']) && ($_['ldap_expert_force_uuid_case'] === 'lowercase')) {
					p(' selected');
				} ?>><?php p($l->t('Convert UUID to Lower Case Group- and Username'));?></option><option value="uppercase"<?php if (isset($_['ldap_expert_force_uuid_case']) && ($_['ldap_expert_force_uuid_case'] === 'uppercase')) {
					p(' selected');
				} ?>><?php p($l->t('Convert UUID to Upper Case Group- Username'));?></option></select>
