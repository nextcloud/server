...
class ShareByMailProviderTest extends TestCase {
	...
	public function testCreateSendPasswordByMailWithEnforcedPasswordProtectionWithPermanentPassword(): void {
		...
		$expectedDispatchedEvents = [
			...
			BeforeSharePasswordMailSentEvent::class,
		];
		$this->eventDispatcher->expects($this->exactly(3))
			->method('dispatchTyped')
			->willReturnCallback(function ($event) use (&$expectedDispatchedEvents): void {
				$this->assertInstanceOf(array_shift($expectedDispatchedEvents), $event);
			});
	...
	public function testSendEmailDoesNotSendWhenMailHandledByListener(): void {
		$provider = $this->getInstance();
		$user = $this->createMock(IUser::class);
		...
		$this->eventDispatcher->expects($this->once())
			->method('dispatchTyped')
			->willReturnCallback(function (BeforeShareMailSentEvent $event): void {
		...
		self::invokePrivate($provider, 'sendEmail', [$share, ['john@doe.com']]);
	}

	public function testSendNoteDoesNotSendWhenMailHandledByListener(): void {
		$provider = $this->getInstance();
		$user = $this->createMock(IUser::class);
		$user->method('getDisplayName')->willReturn('Mrs. Owner User');
		$user->method('getEMailAddress')->willReturn('owner@example.com');
		...
		$this->eventDispatcher->expects($this->once())
			->method('dispatchTyped')
			->willReturnCallback(function (BeforeShareNoteMailSentEvent $event): void {
				$event->markMailHandled();
			});
		$this->mailer->expects($this->never())->method('send');
