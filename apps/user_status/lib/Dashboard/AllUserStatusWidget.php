...
/**
 * Class AllUserStatusWidget
 *
 * @package OCA\UserStatus
 */
class AllUserStatusWidget implements IAPIWidget, IAPIWidgetV2, IIconWidget, IOptionWidget {
	private IL10N $l10n;
	...
	/**
	 * AllUserStatusWidget constructor
	 *
	 * @param IL10N $l10n
	...
	public function getTitle(): string {
		return $this->l10n->t('User statuses');
	}
	...
	private function getWidgetData(string $userId, ?string $since = null, int $limit = 9): array {
		// Fetch status and filter current user
		$userStatuses = array_slice(
			array_filter(
				$this->service->findAll($limit + 1, 0),
	...
	public function getItems(string $userId, ?string $since = null, int $limit = 7): array {
		...
		return array_map(function (array $widgetData) {
			...
			return new WidgetItem(
				...
				$widgetData['icon'] . ($widgetData['icon'] ? ' ' : '') . $widgetData['status'] . ', ' . $formattedDate,
	...
	public function getItemsV2(string $userId, ?string $since = null, int $limit = 7): WidgetItems {
		...
		return new WidgetItems(
			$items,
			count($items) === 0 ? $this->l10n->t('No status retrieved') : '',
		);
	}
