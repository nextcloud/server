		...
		try {
			...
			if ($res === false) {
				if ($this->fileView->file_exists($path)) {
					throw new ServiceUnavailable($this->l10n->t('Could not open file: %1$s (%2$d), file does seem to exist. Last error: %s',
						[$path, $this->info->getId() , json_encode(error_get_last())]));
				} else {
