...
class UploadAutoCollectionPlugin extends ServerPlugin {
	...
	/**
	 * @throws NotFound a node that is expected to exist cannot be found
	 */
	public function beforeMethod(RequestInterface $request, ResponseInterface $response): bool {
		if ($request->getHeader('X-Create-Tree') !== 'enable') {
			return true;
		}
		...
		if (!($node instanceof ICollection)) {
			// somehow the node is not a collection, let SabreDAV handle it
			return true;
		}
