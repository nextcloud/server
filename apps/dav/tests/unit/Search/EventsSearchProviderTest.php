...
class EventsSearchProviderTest extends TestCase {
	...
	// Recurring event with a recurrence fine-grained enough to exceed Sabre's
	// 3500 instance limit while iterating from DTSTART towards the requested
	// range, so expanding it throws MaxInstancesExceededException.
	private static string $vEvent10 = 'BEGIN:VCALENDAR' . PHP_EOL
		. 'VERSION:2.0' . PHP_EOL
		. 'PRODID:-//Tests//' . PHP_EOL
		. 'CALSCALE:GREGORIAN' . PHP_EOL
		. 'BEGIN:VEVENT' . PHP_EOL
		. 'UID:hourly@example.com' . PHP_EOL
		. 'DTSTAMP:20230101T080000Z' . PHP_EOL
		. 'DTSTART;TZID=Europe/Berlin:20230101T090000' . PHP_EOL
		. 'DTEND;TZID=Europe/Berlin:20230101T093000' . PHP_EOL
		. 'RRULE:FREQ=HOURLY' . PHP_EOL
		. 'SUMMARY:Hourly event' . PHP_EOL
		. 'END:VEVENT' . PHP_EOL
		. 'END:VCALENDAR';
...
ICS;
	...
	public function testSearchSince(): void {
		...
		$this->assertEquals('06-01 09:00 - 10:00', $result0Data['subline']);
		$this->assertEquals('deep-link-to-calendar', $result0Data['resourceUrl']);
		$this->assertEquals('icon-calendar-dark', $result0Data['icon']);
		$this->assertFalse($result0Data['rounded']);
	...
	public function testSearchSinceOnlyDefaultsUntilToThreeYearsLater(): void {
		// No until: expanding recurrences still needs a concrete upper bound,
		// so it defaults to three years after since.
		$user = $this->createMock(IUser::class);
		$user->method('getUID')->willReturn('john.doe');
		$query = $this->createMock(ISearchQuery::class);
		$query->method('getFilter')->willReturnCallback(function ($name) {
		...
		$this->backend->expects($this->once())
			...
			->with('principals/users/john.doe', 'search term', ['VEVENT'],
				...
				['limit' => 5, 'offset' => 20, 'timerange' => [
					'start' => $since,
					'end' => new \DateTimeImmutable('2029-05-15 00:00:00'),
				]])
			->willReturn([]);
	...
	public function testSearchUntilOnlyDefaultsSinceToThreeYearsEarlier(): void {
		// No since: the lower bound defaults to three years before until for
		// the same reason.
		$user = $this->createMock(IUser::class);
		$user->method('getUID')->willReturn('john.doe');
		$query = $this->createMock(ISearchQuery::class);
		...
		$this->backend->expects($this->once())
			...
			->with('principals/users/john.doe', 'search term', ['VEVENT'],
				...
				['ATTENDEE' => ['CN'], 'ORGANIZER' => ['CN']],
				['limit' => 5, 'offset' => 20, 'timerange' => [
					'start' => new \DateTimeImmutable('2023-06-14 00:00:00'),
	...
	public function testResolveComponentReturnsNullWhenRecurrenceLimitExceeded(): void {
		// Sabre counts generated instances from DTSTART, not from the start of
		// the requested range, so an hourly event older than 3500 hours blows
		// the limit even for a one-day range. That must drop the row rather
		// than fail the whole search request.
		$since = new \DateTimeImmutable('2026-03-10T00:00:00Z');
		$until = new \DateTimeImmutable('2026-03-11T00:00:00Z');

		$actual = self::invokePrivate($this->provider, 'resolveComponent', [self::$vEvent10, $since, $until]);
