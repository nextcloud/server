...
class RefreshWebcalServiceTest extends TestCase {
	...
	public function testDtstampChangeDoesNotTriggerUpdate(): void {
		...
		// The stored etag was computed from the base component's UID, SEQUENCE and LAST-MODIFIED, excluding DTSTAMP
		$base = VObject\Reader::read($body)->getBaseComponent();
		$existingEtag = md5($base->UID?->getValue() . $base->SEQUENCE?->getValue() . $base->{'LAST-MODIFIED'}?->getValue());
	...
	public function testFoldedDtstampChangeDoesNotTriggerUpdate(): void {
		...
		// Compute etag from the base component's UID, SEQUENCE and LAST-MODIFIED, excluding DTSTAMP
		$base = VObject\Reader::read($body)->getBaseComponent();
		$existingEtag = md5($base->UID?->getValue() . $base->SEQUENCE?->getValue() . $base->{'LAST-MODIFIED'}?->getValue());
	...
	public function testSequenceChangeTriggersUpdate(): void {
		$refreshWebcalService = new RefreshWebcalService(
			$this->caldavBackend,
			$this->logger,
			$this->connection,
		...
		// Feed body has a new SEQUENCE, an actual content change
		$body = "BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//Test//Test//EN\r\nBEGIN:VEVENT\r\nUID:sequence-test\r\nSEQUENCE:2\r\nDTSTAMP:20260209T120000Z\r\nDTSTART:20260301T100000Z\r\nSUMMARY:Test Event\r\nEND:VEVENT\r\nEND:VCALENDAR\r\n";
		$stream = $this->createStreamFromString($body);
		...
		// The stored etag reflects the previous SEQUENCE value
		$existingEtag = md5('sequence-test' . '1');
		...
		// SEQUENCE change must trigger an update
		$this->caldavBackend->expects(self::once())
			->method('updateCalendarObject')
			->with(42, 'sequence-test.ics', $vCalendar->serialize(), CalDavBackend::CALENDAR_TYPE_SUBSCRIPTION);
	...
	public static function identicalDataProvider(): array {
		...
		// Etag is computed from the base component's UID, SEQUENCE and LAST-MODIFIED
		$base = VObject\Reader::read($icalBody)->getBaseComponent();
		$etag = md5($base->UID?->getValue() . $base->SEQUENCE?->getValue() . $base->{'LAST-MODIFIED'}?->getValue());
