OC.L10N.register(
    "appstore",
    {
    "App store" : "Siopa aip",
    "Apps" : "Feidhmchláir",
    "Nextcloud Appstore" : "Siopa Aipeanna Nextcloud",
    "Appstore" : "Siopa Aipeanna",
    "App name" : "Ainm aip",
    "Version" : "Leagan",
    "Support level" : "Leibhéal tacaíochta",
    "Groups" : "Grúpaí",
    "Actions" : "Gníomhartha",
    "Show details" : "Sonraí a thaispeáint",
    "is loading…" : "ag lódáil…",
    "Filter view" : "Radharc scagaire",
    "Grid size" : "Méid an ghreille",
    "Small grid size" : "Méid beag eangaí",
    "Medium grid size" : "Méid greille meánach",
    "Large grid size" : "Méid eangach mór",
    "Show incompatible" : "Taispeáin neamh-chomhoiriúnach",
    "Grid view" : "Radharc greille",
    "Force enable {suite}?" : "Cumasaigh {suite} iallach?",
    "Enabling {suite} requires force enabling the app. This may cause issues with your Nextcloud instance. Are you sure you want to proceed?" : "Éilíonn cumasú {suite} go gcuirfí an aip i ngníomh go foréigneach. D’fhéadfadh fadhbanna a bheith mar thoradh air seo le d’eispéireas Nextcloud. An bhfuil tú cinnte gur mian leat dul ar aghaidh?",
    "Force enable" : "Cumasaigh le fórsa",
    "Cancel" : "Cealaigh",
    "Office suite switching is managed through the Nextcloud All-in-One interface." : "Déantar aistriú idir sraitheanna oifige a bhainistiú tríd an gcomhéadan Nextcloud All-in-One.",
    "Please use the AIO interface to switch between office suites." : "Bain úsáid as an gcomhéadan AIO le do thoil chun aistriú idir shraith oifige.",
    "Select your preferred office suite." : "Roghnaigh do shraith oifige is fearr leat.",
    "Please note that installing requires manual server setup." : "Tabhair faoi deara go bhfuil gá le socrú an fhreastalaí de láimh le haghaidh suiteáil.",
    "Disable office suites" : "Díchumasaigh sraitheanna oifige",
    "installed" : "suiteáilte",
    "Features" : "Gnéithe",
    "Learn more" : "Foghlaim níos mó",
    "Daemon" : "Deamhan",
    "Deploy Daemon" : "Imscaradh Deamhan",
    "Type" : "Cineál",
    "Name" : "Ainm",
    "Display Name" : "Ainm taispeána",
    "GPUs support" : "Tacaíocht GPUs",
    "Compute device" : "Gléas ríomh",
    "Advanced deploy options" : "Ardroghanna imscaradh",
    "Edit ExApp deploy options before installation" : "Cuir roghanna imlonnaithe ExApp in eagar roimh an tsuiteáil",
    "Configured ExApp deploy options. Can be set only during installation" : "Roghanna imlonnaithe ExApp cumraithe. Ní féidir é a shocrú ach amháin le linn na suiteála",
    "Environment variables" : "Athróga timpeallachta",
    "ExApp container environment variables" : "Athróga timpeallachta coimeádán ExApp",
    "No environment variables defined" : "Níl aon athróg timpeallachta sainithe",
    "Mounts" : "Feistis",
    "Define host folder mounts to bind to the ExApp container" : "Sainmhínigh gléasanna an fhillteáin óstaigh chun ceangal leis an gcoimeádán ExApp",
    "Must exist on the Deploy daemon host prior to installing the ExApp" : "Ní mór a bheith ar an ósta Deploy Deploy roimh shuiteáil an ExApp",
    "Host path" : "Cosán óstach",
    "Container path" : "Cosán coimeádán",
    "Read-only" : "Inléite-amháin",
    "Remove mount" : "Bain feisteas",
    "New mount" : "Feisteas nua",
    "Enter path to host folder" : "Cuir isteach cosán chuig an bhfillteán óstach",
    "Enter path to container folder" : "Cuir isteach an cosán chuig an bhfillteán coimeádán",
    "Toggle read-only mode" : "Scoránaigh mód inléite-amháin",
    "Confirm adding new mount" : "Deimhnigh feisteas nua a chur leis",
    "Confirm" : "Deimhnigh",
    "Cancel adding mount" : "Cealaigh feisteas a chur leis",
    "Add mount" : "Cuir feisteas",
    "ExApp container mounts" : "Feisteas coimeádán ExApp",
    "No mounts defined" : "Níl aon feisteas sainithe",
    "Description" : "Cur síos",
    "View in store" : "Amharc sa siopa",
    "Visit website" : "Tabhair cuairt ar an suíomh Gréasáin",
    "Usage documentation" : "Doiciméadú úsáide",
    "Admin documentation" : "Doiciméadúchán riaracháin",
    "Developer documentation" : "Doiciméadú forbróra",
    "Details" : "Sonraí",
    "This app cannot be installed because the following dependencies are not fulfilled:" : "Ní féidir an aip seo a shuiteáil toisc nach bhfuil na spleáchais seo a leanas comhlíonta:",
    "Missing dependencies" : "Spleáchais ar iarraidh",
    "This app cannot be limited to groups because it provides functionality that is executed before group membership is determined." : "Ní féidir an aip seo a theorannú do ghrúpaí mar go soláthraíonn sí feidhmiúlacht a chuirtear i gcrích sula gcinntear ballraíocht an ghrúpa.",
    "Limited to groups" : "Teoranta do ghrúpaí",
    "Latest updated" : "Nuashonraithe is déanaí",
    "Author" : "Údar",
    "Categories" : "Catagóirí",
    "Resources" : "Acmhainní",
    "Documentation resources" : "Acmhainní doiciméadachta",
    "Changelog" : "Loga athrú",
    "Supported" : "Tacaithe",
    "Featured" : "Feiceálach",
    "This app is supported via your current Nextcloud subscription." : "Tacaítear leis an aip seo trí do shíntiús reatha Nextcloud.",
    "Featured apps are developed by and within the community. They offer central functionality and are ready for production use." : "Forbraíonn an pobal agus laistigh den phobal aipeanna sainghnéitheacha. Cuireann siad feidhmiúlacht lárnach ar fáil agus tá siad réidh le húsáid táirgeachta.",
    "Community rating: {score}/5" : "Rátáil pobail: {score}/5",
    "Choose Deploy Daemon for {appName}" : "Roghnaigh Imscaradh Daemon do {appName}",
    "No Deploy daemons configured" : "Níl aon deamhan Imscaradh cumraithe",
    "Register a custom one or setup from available templates" : "Cláraigh ceann nó socrú saincheaptha ó na teimpléid atá ar fáil",
    "Manage Deploy daemons" : "Bainistigh deamhain imscartha",
    "Registered Deploy daemons list" : "Liosta deamhan Imscaradh Cláraithe",
    "Default" : "Réamhshocrú",
    "Carousel" : "Timpealláin",
    "Previous slide" : "Sleamhnán roimhe seo",
    "Next slide" : "Sleamhnán seo chugainn",
    "Choose slide to display" : "Roghnaigh sleamhnán le taispeáint",
    "{index} of {total}" : "{index} de {total}",
    "Limit to groups" : "Teorainn do ghrúpaí",
    "Restrict the usage of {app} to members of the following groups." : "Srian a chur le húsáid {app} do bhaill de na grúpaí seo a leanas.",
    "Reset limitation" : "Athshocraigh an teorainn",
    "Save" : "Sábháil",
    "Failed to update app {appName}" : "Theip ar an aip {appName} a nuashonrú",
    "Update all apps" : "Nuashonraigh gach aip",
    "Are you sure you want to update all apps?" : "An bhfuil tú cinnte gur mhaith leat na haipeanna go léir a nuashonrú?",
    "Update done" : "Nuashonrú déanta",
    "View website" : "Féach ar an suíomh gréasáin",
    "Update all" : "Nuashonraigh go léir",
    "Loading app list" : "Liosta feidhmchlár á lódáil",
    "No matching apps found" : "Ní bhfuarthas aon aipeanna comhoiriúnacha",
    "Search everywhere" : "Cuardaigh i ngach áit",
    "Download and enable all" : "Íoslódáil agus a chumasú go léir",
    "Could not load app discover section" : "Níorbh fhéidir an rannán aimsithe feidhmchláir a lódáil",
    "Could not render element" : "Níorbh fhéidir an eilimint a thabhairt",
    "Nothing to show" : "Ní dhéanfaidh aon ní a thaispeáint",
    "Could not load section content from app store." : "Níorbh fhéidir ábhar na rannóige a lódáil ón appstore.",
    "Loading" : "Ag lódáil",
    "Fetching the latest news…" : "An nuacht is déanaí á bhfáil…",
    "Update all applications" : "Nuashonraigh gach feidhmchlár",
    "Appstore categories" : "Catagóirí Siopa Aipeanna",
    "Search apps…" : "Cuardaigh aipeanna…",
    "Loading categories" : "Catagóirí á lódáil",
    "Please enter more characters to search." : "Cuir isteach níos mó carachtair le cuardach a dhéanamh.",
    "Search apps" : "Cuardaigh aipeanna",
    "Version {version}, {license}-licensed" : "Leagan {version}, {license}-cheadúnaithe",
    "Version {version}" : "Leagan {version}",
    "Disable" : "Díchumasaigh",
    "Enable" : "Cumasaigh",
    "Deploy and enable" : "Imscaradh agus cumasú",
    "Download and enable" : "Íoslódáil agus a chumasú",
    "Install and enable" : "Suiteáil agus cumasaigh",
    "Deploy and force enable" : "Imscaradh agus cumasú fórsaithe",
    "Download and force enable" : "Íoslódáil agus cumasaigh iallach",
    "Install and force enable" : "Suiteáil agus cumasú fórsaithe",
    "Rate the app" : "Ráta an app",
    "Report a bug" : "Tuairiscigh fabht",
    "Ask questions or discuss the app" : "Cuir ceisteanna nó pléigh an aip",
    "Visit the website" : "Tabhair cuairt ar an suíomh gréasáin",
    "Remove" : "Bain",
    "Update to {version}" : "Nuashonraigh go {version}",
    "Discover" : "Faigh amach",
    "Your apps" : "D'aipeanna",
    "Active apps" : "Aipeanna gníomhacha",
    "Disabled apps" : "Aipeanna díchumasaithe",
    "Updates" : "Nuashonruithe",
    "App bundles" : "Pacáistí aip",
    "Featured apps" : "Aipeanna faoi thrácht",
    "Supported apps" : "Aipeanna tacaithe",
    "Search results" : "Torthaí cuardaigh",
    "Powered by Euro-Office" : "Cumhachtaithe ag Euro-Office",
    "Good Nextcloud integration" : "Comhtháthú maith Nextcloud",
    "Open source" : "Foinse oscailte",
    "Best performance" : "An fheidhmíocht is fearr",
    "Limited ODF compatibility" : "Comhoiriúnacht ODF teoranta",
    "Best Microsoft compatibility" : "Comhoiriúnacht is fearr le Microsoft",
    "Best Nextcloud integration" : "Comhtháthú Nextcloud is fearr",
    "Good performance" : "Feidhmíocht mhaith",
    "Best security: documents never leave your server" : "An tslándáil is fearr: ní fhágann doiciméid do fhreastalaí choíche",
    "Best ODF compatibility" : "Comhoiriúnacht OF is fearr",
    "Best support for legacy files" : "An tacaíocht is fearr do chomhaid oidhreachta",
    "Could not load app categories. Please try again later." : "Níorbh fhéidir catagóirí aipeanna a lódáil. Déan iarracht arís ar ball.",
    "Could not load apps list. Please try again later." : "Níorbh fhéidir liosta na n-aipeanna a lódáil. Déan iarracht arís ar ball.",
    "Could not update the app. Please try again later." : "Níorbh fhéidir an aip a nuashonrú. Déan iarracht arís ar ball.",
    "An error occurred during the request. Unable to proceed." : "Tharla earráid le linn an iarratais. Ní féidir dul ar aghaidh."
},
"nplurals=5; plural=(n==1 ? 0 : n==2 ? 1 : n<7 ? 2 : n<11 ? 3 : 4);");
