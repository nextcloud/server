OC.L10N.register(
    "appstore",
    {
    "App store" : "应用商店",
    "Apps" : "应用",
    "Nextcloud Appstore" : "Nextcloud 应用商店",
    "App name" : "应用的名称",
    "Version" : "版本",
    "Support level" : "支持级别",
    "Groups" : "群组",
    "Actions" : "操作",
    "Show details" : "显示详情",
    "is loading…" : "正在加载…",
    "Filter view" : "筛选视图",
    "Grid size" : "网格大小",
    "Small grid size" : "小网格尺寸",
    "Medium grid size" : "中网格尺寸",
    "Large grid size" : "大网格尺寸",
    "Show incompatible" : "显示不兼容",
    "Grid view" : "网格视图",
    "Force enable {suite}?" : "强制启用 {suite}？",
    "Enabling {suite} requires force enabling the app. This may cause issues with your Nextcloud instance. Are you sure you want to proceed?" : "启用 {suite} 需要强制启用该应用。这可能会导致您的 Nextcloud 实例出现问题。是否确定要继续？",
    "Force enable" : "强制启用",
    "Cancel" : "取消",
    "Office suite switching is managed through the Nextcloud All-in-One interface." : "办公套件的切换是通过 Nextcloud All-in-One 界面进行管理的。",
    "Please use the AIO interface to switch between office suites." : "请使用 AIO 界面在办公套件之间切换。",
    "Select your preferred office suite." : "选择您的首选办公套件。",
    "Please note that installing requires manual server setup." : "请注意，安装需要手动设置服务器。",
    "Disable office suites" : "禁用办公套件",
    "installed" : "已安装",
    "Features" : "功能",
    "Learn more" : "了解更多",
    "Daemon" : "守护进程",
    "Deploy Daemon" : "部署守护进程",
    "Type" : "类型",
    "Name" : "名称",
    "Display Name" : "显示名称",
    "GPUs support" : "GPU 支持",
    "Compute device" : "计算设备",
    "Advanced deploy options" : "高级部署选项",
    "Edit ExApp deploy options before installation" : "安装前编辑外部应用部署选项",
    "Configured ExApp deploy options. Can be set only during installation" : "已配置外部应用部署选项，只能在安装过程中设置",
    "Environment variables" : "环境变量",
    "ExApp container environment variables" : "外部应用容器环境变量",
    "No environment variables defined" : "未定义环境变量",
    "Mounts" : "挂载",
    "Define host folder mounts to bind to the ExApp container" : "定义要绑定到外部应用容器的主机文件夹挂载",
    "Must exist on the Deploy daemon host prior to installing the ExApp" : "安装外部应用之前，必须存在于部署守护进程主机上",
    "Host path" : "主机路径",
    "Container path" : "容器路径",
    "Read-only" : "只读",
    "Remove mount" : "移除挂载",
    "New mount" : "新挂载",
    "Enter path to host folder" : "输入主机文件夹路径",
    "Enter path to container folder" : "输入容器文件夹的路径",
    "Toggle read-only mode" : "切换只读模式",
    "Confirm adding new mount" : "确认添加新挂载",
    "Confirm" : "确认",
    "Cancel adding mount" : "取消添加挂载",
    "Add mount" : "添加挂载",
    "ExApp container mounts" : "外部应用容器挂载",
    "No mounts defined" : "未定义挂载",
    "Description" : "描述",
    "View in store" : "在商店中查看",
    "Visit website" : "访问网站",
    "Usage documentation" : "使用手册",
    "Admin documentation" : "管理员文档",
    "Developer documentation" : "开发者文档",
    "Details" : "详情",
    "This app cannot be installed because the following dependencies are not fulfilled:" : "无法安装应用，因为无法满足下列依赖：",
    "Missing dependencies" : "缺少依赖",
    "This app cannot be limited to groups because it provides functionality that is executed before group membership is determined." : "此应用无法限定于群组，因为其提供的功能会在群组成员判定前执行。",
    "Limited to groups" : "仅限于组",
    "Latest updated" : "最近更新",
    "Author" : "作者",
    "Categories" : "分类",
    "Resources" : "资源",
    "Documentation resources" : "文档资源",
    "Changelog" : "更新日志",
    "Supported" : "受支持",
    "Featured" : "精选",
    "This app is supported via your current Nextcloud subscription." : "根据你当前的 Nextcloud 订阅，此应用受到支持。",
    "Featured apps are developed by and within the community. They offer central functionality and are ready for production use." : "特色应用由社区并在社区内开发。 它们提供了中心功能，并准备投入生产使用。",
    "Community rating: {score}/5" : "社区评分：{score}/5",
    "Choose Deploy Daemon for {appName}" : "为 {appName} 选择部署守护进程",
    "No Deploy daemons configured" : "没有部署守护进程配置",
    "Register a custom one or setup from available templates" : "注册自定义模板或从可用模板进行安装",
    "Manage Deploy daemons" : "管理部署守护进程",
    "Registered Deploy daemons list" : "已注册的部署守护进程列表",
    "Default" : "默认",
    "Carousel" : "轮播",
    "Previous slide" : "上一张幻灯片",
    "Next slide" : "下一张幻灯片",
    "Choose slide to display" : "选择要显示的幻灯片",
    "{index} of {total}" : "第 {index} 页，共 {total} 页",
    "Limit to groups" : "限制组",
    "Restrict the usage of {app} to members of the following groups." : "将 {app} 的使用限制为以下组的成员。",
    "Reset limitation" : "重置限制",
    "Save" : "保存",
    "Update all" : "全部更新",
    "Loading app list" : "正在加载应用程序列表",
    "No matching apps found" : "未找到匹配应用",
    "Search everywhere" : "全局搜索",
    "Download and enable all" : "下载并启用全部",
    "Could not load app discover section" : "无法加载应用程序发现部分",
    "Could not render element" : "无法渲染元素",
    "Nothing to show" : "没有什么可显示的",
    "Could not load section content from app store." : "无法从应用商店加载部分内容。",
    "Loading" : "加载中",
    "Fetching the latest news…" : "正在获取最新消息...",
    "Appstore categories" : "应用商店类别",
    "Search apps…" : "搜索应用…",
    "Loading categories" : "正在加载分类",
    "Please enter more characters to search." : "请输入更多字符进行搜索。",
    "Search apps" : "搜索应用",
    "Version {version}, {license}-licensed" : "版本 {version}，授权：{license}",
    "Version {version}" : "版本 {version}",
    "Disable" : "禁用",
    "Enable" : "启用",
    "Deploy and enable" : "部署并启用",
    "Download and enable" : "下载并启用",
    "Install and enable" : "安装并启用",
    "Deploy and force enable" : "部署并强制启用",
    "Download and force enable" : "下载并强制启用",
    "Install and force enable" : "安装并强制启用",
    "Rate the app" : "对应用进行评分",
    "Report a bug" : "报告 Bug",
    "Ask questions or discuss the app" : "提问或讨论应用",
    "Visit the website" : "访问网站",
    "Remove" : "移除",
    "Update to {version}" : "更新至 {version}",
    "Discover" : "发现",
    "Your apps" : "你的应用",
    "Active apps" : "已启用的应用",
    "Disabled apps" : "已禁用的应用",
    "Updates" : "更新",
    "App bundles" : "应用捆绑包",
    "Featured apps" : "精选应用",
    "Supported apps" : "支持的应用",
    "Search results" : "搜索结果",
    "Good Nextcloud integration" : "Nextcloud 集成良好",
    "Open source" : "开源",
    "Best performance" : "最佳性能",
    "Limited ODF compatibility" : "ODF 兼容性有限",
    "Best Microsoft compatibility" : "最佳 Microsoft 兼容性",
    "Best Nextcloud integration" : "最佳 Nextcloud 集成",
    "Good performance" : "性能良好",
    "Best security: documents never leave your server" : "最佳安全性：文档永远不会离开您的服务器",
    "Best ODF compatibility" : "最佳 ODF 兼容性",
    "Best support for legacy files" : "对旧版文件的最佳支持",
    "Could not load app categories. Please try again later." : "无法加载应用类别。请稍后重试。",
    "Could not load apps list. Please try again later." : "无法加载应用列表。请稍后重试。",
    "Could not update the app. Please try again later." : "无法更新应用。请稍后重试。",
    "An error occurred during the request. Unable to proceed." : "请求期间发生错误。无法继续。"
},
"nplurals=1; plural=0;");
