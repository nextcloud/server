...
const storage = (0,_nextcloud_browser_storage__WEBPACK_IMPORTED_MODULE_1__.getBuilder)('files_sharing').build();
...
function registerFileRequestHeader(nickname) {
  const uploader = (0,_nextcloud_upload__WEBPACK_IMPORTED_MODULE_6__.getUploader)();
  uploader.setCustomHeader('X-NC-Nickname', encodeURIComponent(nickname));
  _services_logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].debug('Nickname header registered for uploader', {
 ...
 * @param guest
 */
function onUserInfoChanged(guest) {
  _services_logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].debug('User info changed', {
    guest
  });
  registerFileRequestHeader(guest.displayName ?? '');
}
// Monitor nickname changes
(0,_nextcloud_event_bus__WEBPACK_IMPORTED_MODULE_3__.subscribe)('user:info:changed', onUserInfoChanged);
window.addEventListener('DOMContentLoaded', () => {
  ...
  const isFileRequest = (0,_nextcloud_initial_state__WEBPACK_IMPORTED_MODULE_4__.loadState)('files_sharing', 'isFileRequest', false);
  const owner = (0,_nextcloud_initial_state__WEBPACK_IMPORTED_MODULE_4__.loadState)('files_sharing', 'owner', '');
  const ownerDisplayName = (0,_nextcloud_initial_state__WEBPACK_IMPORTED_MODULE_4__.loadState)('files_sharing', 'ownerDisplayName', '');
  const label = (0,_nextcloud_initial_state__WEBPACK_IMPORTED_MODULE_4__.loadState)('files_sharing', 'label', '');
  const filename = (0,_nextcloud_initial_state__WEBPACK_IMPORTED_MODULE_4__.loadState)('files_sharing', 'filename', '');
  ...
  if (isFileRequest) {
    ...
    if (!nickname || !dialogShown) {
      _services_logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].debug('Showing public auth prompt.', {
        nickname
      });
      (0,_nextcloud_dialogs__WEBPACK_IMPORTED_MODULE_2__.showGuestUserPrompt)(options);
    }
    return;
  }
  if (!dialogShown && !nickname) {
    _services_logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].debug('Public auth prompt not shown yet but nickname is not mandatory.', {
      nickname
    });
    return;
  }
  // Else, we just register the nickname header if any.
  _services_logger_ts__WEBPACK_IMPORTED_MODULE_7__["default"].debug('Public auth prompt already shown.', {
    nickname
  });
...
/******/ })()
;
//# sourceMappingURL=files_sharing-public-nickname-handler.js.map?v=961e75a4b446eb06ce53