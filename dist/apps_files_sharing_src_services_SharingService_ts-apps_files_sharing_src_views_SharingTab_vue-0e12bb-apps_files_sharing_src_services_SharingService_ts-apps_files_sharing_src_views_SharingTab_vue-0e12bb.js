...
const ocsEntryToNode = async function (ocsEntry) {
  try {
    ...
    const expiration = ocsEntry?.expiration ? new Date(ocsEntry?.expiration)?.getTime() / 1000 : undefined;
    return new Node({
      id: fileid,
      source,
...
const getExpiredShares = function () {
  const url = (0,_nextcloud_router__WEBPACK_IMPORTED_MODULE_2__.generateOcsUrl)('apps/files_sharing/api/v1/expiredshares');
  return _nextcloud_axios__WEBPACK_IMPORTED_MODULE_3__["default"].get(url, {
    headers,
    params: {
      include_tags: true
...
const getContents = async function () {
  ...
  let expiredShares = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
  let filterTypes = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : [];
  const promises = [];
  if (sharedWithYou) {
...
//# sourceMappingURL=apps_files_sharing_src_services_SharingService_ts-apps_files_sharing_src_views_SharingTab_vue-0e12bb-apps_files_sharing_src_services_SharingService_ts-apps_files_sharing_src_views_SharingTab_vue-0e12bb.js.map?v=ba0f5170dbe5011a8ec7