...
']
	...
	protected function setUp() {
		...
		$this->config
			...
			->will($this->returnCallback(function($key,$bool=false) {
		...
		$user->expects($this->any())
			->method('getHome')
			->willReturn($this->testDir."testuser/");
	...
	public function testEncrypt($uid) {
		...
		$crypttext = ''; //FIXME good test
		if ($uid === "without") {
			$this->gpg->import($this->testKeys[0]['pubkey']);
			$text = $this->gpg->encrypt([$this->testKeys[0]['fingerprint']],$plaintext);
		} else {

			$this->gpg->import($this->testKeys[0]['pubkey'],$uid);
			$text = $this->gpg->encrypt([$this->testKeys[0]['fingerprint']],$plaintext, $uid);
		}
		$this->assertEquals(699,strlen($text));
	}
	 ...
	 * @param $uid = null
	 */
	public function testEncryptsign(array $encrypt_fingerprints, array $sign_fingerprints, $uid, $result) {
		$plaintext = "This is a test Text";
		if ($uid === "without") {
			foreach($this->testKeys as $key){
				if(in_array($key['fingerprint'],$sign_fingerprints) || array_key_exists($key['fingerprint'], $sign_fingerprints)){
					$this->gpg->import($key['privkey']);
				}
				if(in_array($key['fingerprint'],$encrypt_fingerprints)){
					$this->gpg->import($key['pubkey']);
				}
			}
			...
			foreach($this->testKeys as $key){
				if(in_array($key['fingerprint'],$sign_fingerprints) || array_key_exists($key['fingerprint'], $sign_fingerprints)){
					$this->gpg->import($key['privkey'], $uid);
				}
				if(in_array($key['fingerprint'],$encrypt_fingerprints)){
					$this->gpg->import($key['pubkey'], $uid);
				}
			}
			$text = $this->gpg->encryptsign( $encrypt_fingerprints, $sign_fingerprints, $plaintext, $uid);
		}
		$this->assertTrue($text !== $plaintext);
		//$this->assertEquals($result ,strlen($text)); Cryptext is chaning length
	}
	...
	public function testSign($uid) {
		...
		$crypttext = ''; //FIXME good test
		if ($uid === "without") {
			$this->gpg->import($this->testKeys[0]['privkey']);
			$text = $this->gpg->sign([$this->testKeys[0]['fingerprint']],$plaintext);
		} else {

			$this->gpg->import($this->testKeys[0]['privkey'],$uid);
			$text = $this->gpg->sign([$this->testKeys[0]['fingerprint']],$plaintext, $uid);
		}
		$this->assertEquals(659,strlen($text));
	}
	...
	public function dataImport(){
		...
		$testSecretWithPassphraseKeyResult = [
			...
			'secretimported' => 1,
			'skippedkeys' => 0,
			'fingerprint' => $this->testKeys[1]['fingerprint']
		];
		return [
	...
	public function testExport( $uid  ) {
		$testKey = $this->testKeys[0]['pubkey'];
		$testKeyResult = [
			'imported' => 1,
			'unchanged' => 0,
			'secretimported' => 0,
		...
		foreach($testKeyResult as $key => $value){
			$this->assertEquals($value,$result[$key]);
		}

		if ($uid === 'without'){
			$key = $this->gpg->export($result['fingerprint']);
		} else {
			$key = $this->gpg->export($result['fingerprint'], $uid);
		}

		//$this->assertEquals(strlen($testKey),strlen($key));
		$this->assertEquals(substr_count($testKey,"\n"),substr_count($key,"\n"));
		$this->assertEquals(substr_count($testKey,"a"),substr_count($key,"a"));
		$this->assertEquals(substr_count($testKey,"D"),substr_count($key,"D"));
	 ...
	 * @param string|null $uid
	 */
	public function testKeyinfo($keys, $pattern , $uid, $result) {
		if ($uid === 'without'){
			foreach($keys as $key){
				$this->gpg->import($key);
			}
		...
		//print_r($r);
		if(count($result) !== 1){
			$this->assertEquals(count($result),count($r));
		} else {
			foreach($result as $n => $key){
				foreach($key as $k => $v) {
					if($k === 'uids' || $k === 'subkeys'){
						if (False /* if there are problems with order count($v)!==1'*/) {
							$this->assertEquals(count($v),count($r[$n][$k]));
						} else {
							foreach($v as $nu => $u) {
								foreach($u as $a => $b){
									$this->assertEquals($b,$r[$n][$k][$nu][$a]);
								}
							}
						}
					} else {
						$this->assertEquals($v,$r[$n][$k]);
					}
				}
			}
		}
	...
	public function dataKeyinfo(){
		$wernerSearchResult = Array([
				'expired' => FALSE,
				'revoked' => FALSE,
				'is_secret' => FALSE,
				'uids' => [['name' => 'Werner']],
				'subkeys' => [['fingerprint' => $this->testKeys[1]['fingerprint']]]
		]);
		return [
			[[], "", null, []],
			[[], "", 'testuser', []],
			[[$this->testKeys[0]['pubkey'],$this->testKeys[1]['pubkey']], "Werner", null, $wernerSearchResult],
			[[$this->testKeys[0]['pubkey'],$this->testKeys[1]['pubkey']], "Werner", 'without', $wernerSearchResult],
			[[$this->testKeys[0]['pubkey'],$this->testKeys[1]['pubkey']], "Werner", 'testuser', $wernerSearchResult],
			[[$this->testKeys[0]['pubkey'],$this->testKeys[1]['pubkey']], "", null, [1,2]],
		];
	}
	...
	public function testDelete( $uid  ) {
		if ($uid === 'without') {
			$fingerprint = $this->gpg->import($this->testKeys[0]['pubkey'])['fingerprint'];
			$this->assertEquals($this->testKeys[0]['fingerprint'],$fingerprint);
			$this->assertEquals(1,count($this->gpg->keyinfo($fingerprint)));
			$this->gpg->deletekey($fingerprint);
			$this->assertEquals(0,count($this->gpg->keyinfo($fingerprint)));
		} else {
			$fingerprint = $this->gpg->import($this->testKeys[0]['pubkey'],$uid)['fingerprint'];
			$this->assertEquals($this->testKeys[0]['fingerprint'],$fingerprint);
			$this->assertEquals(1,count($this->gpg->keyinfo($fingerprint,$uid)));
			$this->gpg->deletekey($fingerprint, $uid);
			$this->assertEquals(0,count($this->gpg->keyinfo($fingerprint,$uid)));
		}
	}
	 ...
	 */
	public function testGetPublicKeyFromEmail($keys, $email, $uid , $expectFingerprint) {
		if ($uid === 'without') {
			foreach ($keys as $key){
				$this->gpg->import($key);
			}
		...
		$this->assertEquals($expectFingerprint,$fingerprint);
	...
	public function dataGetPublicKeyFromEmail(){
		return [
			[[], "", null, ''],
			[[$this->testKeys[0]['pubkey'],$this->testKeys[1]['pubkey'],$this->testKeys[2]['pubkey']], $this->testKeys[0]['email'], null, $this->testKeys[0]['fingerprint']],
			[[$this->testKeys[0]['pubkey'],$this->testKeys[1]['pubkey'],$this->testKeys[2]['pubkey']], $this->testKeys[0]['email'], 'without', $this->testKeys[0]['fingerprint']],
			[[$this->testKeys[0]['pubkey'],$this->testKeys[1]['pubkey'],$this->testKeys[2]['pubkey']], $this->testKeys[0]['email'], 'testuser', $this->testKeys[0]['fingerprint']],
			[[$this->testKeys[0]['pubkey'],$this->testKeys[1]['pubkey'],$this->testKeys[2]['pubkey']], $this->testKeys[2]['email2'], null, $this->testKeys[2]['fingerprint']],
			[[$this->testKeys[0]['pubkey'],$this->testKeys[1]['pubkey'],$this->testKeys[2]['pubkey']], $this->testKeys[2]['email1'], null, $this->testKeys[2]['fingerprint']],
		];
	}


	/**
	 * @dataProvider dataGetPrivateKeyFromEmail
	 *
	 * @param string|null $uid
	 */
	/*
	public function testGetPrivateKeyFromEmail($keys, $email, $uid ) {

	}

	public function dataGetPrivateKeyFromEmail(){
		return [
			[[], [""], [null], []],
			[[], [""], ['testuser'], []]
			//FIXME more testcases with multiple keys etc
		];
	}
	*/
	...
	public function testGenerateKey($email, $name, $comment, $uid ) {
		$warning_key_not_found = false;
		$info_no_email_or_name = false;
		$this->logger->expects($this->any())
			->method("warning")
			->will($this->returnCallback(function($message) use (&$warning_key_not_found){
				if ($message === "No server GPG key found so no signed emails are possible"){
					$warning_key_not_found = true;
				}
			}));
		$this->logger->expects($this->any())
			->method("info")
			->will($this->returnCallback(function($message) use (&$info_no_email_or_name){
				if ($message === "Creating Key without email or name is not possible"){
					$info_no_email_or_name = true;
				}
			}));
		$fingerprint = $this->gpg->generateKey($email,$name,$comment,$uid);
		if(!$warning_key_not_found && !$info_no_email_or_name){
			$this->assertEquals(40,strlen($fingerprint));
		} else {
			$this->assertEquals(0,strlen($fingerprint));
			return;
		}

		$info = $this->gpg->keyinfo($fingerprint, $uid);
		if ($email === '') {
			/* otherwise setUser(X); generateKey(); Would generate a server key for User X); */
			if ($uid === '' || $uid === null) {
				$name = 'Nextcloud';
				$comment = 'A Safe Home for your data';
			} else {
				$this->assertEquals($email,$info[0]['uids'][0]['email']);
			}
		} else {
			$this->assertEquals($email,$info[0]['uids'][0]['email']);
		}
		$this->assertEquals($name,$info[0]['uids'][0]['name']);
		$this->assertEquals($comment,$info[0]['uids'][0]['comment']);
	}
	 ...
	 *
	 */
	public function testWorkingAsServerAndUser(){
		$this->gpg->import($this->testKeys[0]['pubkey']);
		$this->gpg->import($this->testKeys[1]['pubkey'],'testuser');
		$this->gpg->import($this->testKeys[2]['pubkey'],null);

		$this->assertEquals(1,count($this->gpg->keyinfo("", 'testuser')));
		$this->assertEquals(2,count($this->gpg->keyinfo("")));
		$this->assertEquals([],$this->gpg->keyinfo($this->testKeys[0]['fingerprint'], 'testuser'));
		$this->assertEquals([],$this->gpg->keyinfo($this->testKeys[1]['fingerprint'], null));
	}
	...
	private static function deleteDir($dirPath) {
		...
		$hidden_files = glob($dirPath . '.?*', GLOB_MARK);
