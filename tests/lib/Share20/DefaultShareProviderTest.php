...
class DefaultShareProviderTest extends \Test\TestCase {
	...
	public function testGetShareByIdUserShare() {
		...
		$sharedBy = $this->getMockBuilder('OCP\IUser')->getMock();
		$sharedBy->method('getUID')->willReturn('sharedBy');
		$shareOwner = $this->getMockBuilder('OCP\IUser')->getMock();
		$shareOwner->method('getUID')->willReturn('shareOwner');

		$ownerPath = $this->getMockBuilder('OCP\Files\File')->getMock();
	...
	public function testGetShareByIdUserGroupShare() {
		...
		$user0 = $this->getMockBuilder('OCP\IUser')->getMock();
		$user0->method('getUID')->willReturn('user0');
		$user1 = $this->getMockBuilder('OCP\IUser')->getMock();
		$user1->method('getUID')->willReturn('user1');

		$group0 = $this->getMockBuilder('OCP\IGroup')->getMock();
	...
	public function dataDeleteUser() {
		return [
			...
			[\OCP\Share::SHARE_TYPE_LINK, 'a', 'b', 'c', 'a', false],
			// To avoid invisible link shares delete initiated link shares as well (see #22327)
			[\OCP\Share::SHARE_TYPE_LINK, 'a', 'b', 'c', 'b', false],
