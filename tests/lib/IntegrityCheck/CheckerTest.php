...
class CheckerTest extends TestCase {
	private ServerVersion&MockObject $serverVersionStub;
	private EnvironmentHelper&MockObject $environmentHelperStub;
	private IConfig&MockObject $configStub;
	private ICacheFactory&MockObject $cacheFactoryStub;
	private Detection&MockObject $mimeTypeDetectorStub;

	private FileAccessHelper&MockObject $fileAccessHelperMock;
	private IAppConfig&MockObject $appConfigMock;
	private IAppManager&MockObject $appManagerMock;

	private Checker $checker; // The system under test (SUT)
	...
	protected function setUp(): void {
		...
		$this->configStub = $this->createStub(IConfig::class);
		$this->cacheFactoryStub = $this->createStub(ICacheFactory::class);
		$this->mimeTypeDetectorStub = $this->createStub(Detection::class);

		// Mocks (Predefined Answers + Matching Critereia + Verification Expectations)
		$this->fileAccessHelperMock = $this->createMock(FileAccessHelper::class);
		$this->appConfigMock = $this->createMock(IAppConfig::class);
		$this->appManagerMock = $this->createMock(IAppManager::class);

		// Default stubbed methods (first-match-wins so add more specific first)
		$this->cacheFactoryStub->method('createDistributed')
	...
	private function addTypicalStubs(): void {
		// Common stubbed methods (first-match-wins so add more specific first)
		$this->serverVersionStub->method('getChannel')
			->willReturn('stable');
	...
	private function createRsaAndX509(string $certFile = 'SomeApp.crt', string $keyFile = 'SomeApp.key'): array {
		$keyBundle = file_get_contents($this->fixtureDir . $certFile);
		$rsaPrivateKey = file_get_contents($this->fixtureDir . $keyFile);
		$rsa = new RSA();
	...
	private function expectSignaturePutContents(string $targetPath, string $expectedJson): void {
		$this->fileAccessHelperMock->expects($this->once())
			->method('file_put_contents')
			->with(
				$this->equalTo($targetPath),
				$this->callback(function ($actualJson) use ($expectedJson) {
	...
	public function testWriteAppSignatureOfNotExistingApp(): void {
		...
		[$x509, $rsa] = $this->createRsaAndX509('SomeApp.crt', 'SomeApp.key');
    ...
    "certificate": "-----BEGIN CERTIFICATE-----\r\\r\n4B+pXTAcRK0zECDEaX3npS8xWzrB\r\n-----END CERTIFICATE-----"
		...
		$targetSignature = \OC::$SERVERROOT . '/tests/data/integritycheck/app//appinfo/signature.json';
		$this->expectSignaturePutContents($targetSignature, $expectedSignatureFileData);
		[$x509, $rsa] = $this->createRsaAndX509('SomeApp.crt', 'SomeApp.key');
	...
	public function testVerifyAppSignatureWithValidSignatureData(): void {
		$this->addTypicalStubs();
		$this->appManagerMock->expects($this->once())
			->method('getAppPath')
			->with('SomeApp')
	...
	public function testVerifyAppSignatureWithTamperedFilesAndAlternatePath(): void {
		$this->addTypicalStubs();
		$this->appManagerMock->expects($this->never())
			->method('getAppPath')
			->with('SomeApp');
		$signatureDataFile = '{
    "hashes": {
    ...
    "certificate": "-----BEGIN CERTIFICATE-----\r\\r\nV204fGUkJqW5CrKy3P3XvY9X\r\n-----END CERTIFICATE-----"
		...
		$this->environmentHelperStub->method('getServerRoot')
			->willReturn(\OC::$SERVERROOT . '/tests/data/integritycheck/app/');
		$target = \OC::$SERVERROOT . '/tests/data/integritycheck/app//core/signature.json';

		$this->expectSignaturePutContents($target, $expectedSignatureFileData);
		[$x509, $rsa] = $this->createRsaAndX509('core.crt', 'core.key');
    ...
    "certificate": "-----BEGIN CERTIFICATE-----\r\\r\nV204fGUkJqW5CrKy3P3XvY9X\r\n-----END CERTIFICATE-----"
		...
		$target = \OC::$SERVERROOT . '/tests/data/integritycheck/htaccessWithInvalidModifiedContent//core/signature.json';
	...
	public function testVerifyCoreSignatureWithModifiedMimetypelistSignatureData(): void {
		...
		$this->mimeTypeDetectorStub->method('getOnlyDefaultAliases')
			->willReturn($shippedMimetypeAliases);

		$this->mimeTypeDetectorStub->method('getAllAliases')
			->willReturn($allAliases);

		$this->mimeTypeDetectorStub->method('getAllNamings')
			->willReturn($allMimetypeNames);

		$oldMimetypeList = new GenerateMimetypeFileBuilder();
		$all = $this->mimeTypeDetectorStub->getAllAliases();
		$namings = $this->mimeTypeDetectorStub->getAllNamings();
	...
	public function testRunInstanceVerification(): void {
		...
		$this->appManagerMock->expects($this->exactly(4))
			->method('isShipped')
			->willReturnMap([
				['files', true],
				['calendar', false],
		...
		$this->checker->expects($this->exactly(3))
		...
		$this->appManagerMock->expects($this->exactly(2))
			->method('getAppPath')
			->willReturnMap([
	...
	public function testVerifyAppSignatureWithoutSignatureDataAndCodeCheckerDisabled(): void {
		...
		$this->configStub->method('getSystemValueBool')
			->with('integrity.check.disabled', false)
			->willReturn(true); // Required for this test
		$expected = [];
	...
	public function testIsCodeCheckEnforced($channel, $isCodeSigningEnforced): void {
		$this->serverVersionStub->method('getChannel')
			->willReturn($channel); // Required for this test
		$this->configStub->method('getSystemValueBool')
			->with('integrity.check.disabled', false)
			->willReturn(false);
