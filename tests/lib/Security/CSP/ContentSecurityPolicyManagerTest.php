...
class ContentSecurityPolicyManagerTest extends TestCase {
	...
	public function testGetDefaultPolicyWithPolicies() {
		...
		$expectedStringPolicy = "default-src 'none';base-uri 'none';manifest-src 'self';prefetch-src 'self';script-src 'self' 'unsafe-inline' 'unsafe-eval';style-src 'self' 'unsafe-inline';img-src 'self' data: blob: anotherdomain.de example.org;font-src 'self' data: mydomain.com example.com anotherFontDomain;connect-src 'self';media-src 'self';child-src childdomain;frame-ancestors 'self';form-action 'self' thirdDomain";
