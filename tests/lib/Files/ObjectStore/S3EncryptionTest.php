...
class S3EncryptionTest extends \Test\TestCase {
	...
	public static function dataFileSizes(): array {
		...
		if (getenv('RUN_HEAVY_S3_TESTS')) {
			$sizes['64MB (historical issue)'] = [64 * 1024 * 1024];
			$sizes['100MB (stress test)'] = [100 * 1024 * 1024];
		}
		return $sizes;
	}
	...
	public function testSizeConsistencyAcrossSources(int $originalSize): void {
		...
		// Verify content integrity via hash to avoid holding two large buffers simultaneously
		$this->assertEquals($expectedHash, hash('sha256', $content),
	...
	public function testEncryptedFileRoundTrip(int $size): void {
		...
		// Verify content via hash to avoid holding two large buffers in memory
		$this->assertEquals($expectedHash, hash('sha256', $readData), 'Content mismatch after round trip');
	...
	public function testEncryptedFileIntegrity(int $size): void {
		...
		// Stream read using incremental hashing to avoid large in-memory buffers
		...
		// Verify
		$this->assertEquals($size, $totalRead, 'Size mismatch in streaming read');
		$this->assertEquals($expectedHash, hash_final($ctx), 'Content mismatch in streaming read');
	...
	public function testEncryptedMultipartUpload(): void {
		if (!getenv('RUN_HEAVY_S3_TESTS')) {
			$this->markTestSkipped('Set RUN_HEAVY_S3_TESTS=1 to run large-file multipart tests');
		}

		$testFile = 'test-multipart.bin';
		// 110 MiB file to exceed the multipart upload threshold (~100 MiB); see S3 backend source.
		$size = 110 * 1024 * 1024;
		$data = random_bytes($size);
		$expectedHash = hash('sha256', $data);
		...
		// Verify content via streaming hash to avoid loading 110MiB into memory
		$stream = $this->view->fopen($testFile, 'rb');
		$this->assertNotFalse($stream, 'Failed to open multipart encrypted upload for reading');
		...
		fclose($stream);
		$this->assertEquals($expectedHash, hash_final($hashCtx),
