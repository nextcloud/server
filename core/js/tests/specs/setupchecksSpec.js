...
describe('OC.SetupChecks tests', function() {
	...
	describe('checkSetup', function() {
		...
		it('should return an error if /dev/urandom is not accessible', function(done) {
			...
			async.done(function( data, s, x ){
				expect(data).toEqual([{
					msg: 'No suitable source for randomness found by PHP which is highly discouraged for security reasons. Further information can be found in the <a target="_blank" rel="noreferrer noopener" class="external" href="https://docs.owncloud.org/myDocs.html">documentation ↗</a>.',
					type: OC.SetupChecks.MESSAGE_TYPE_ERROR
				}]);
				done();
			});
		...
		it('should return an info if server has no proper opcache', function(done) {
			...
			async.done(function( data, s, x ){
				expect(data).toEqual([{
						msg: 'The PHP OPcache is not properly configured. <a target="_blank" rel="noreferrer noopener" class="external" href="https://example.org/link/to/doc">For better performance it is recommended ↗</a> to use the following settings in the <code>php.ini</code>:' + "<pre><code>opcache.enable=1\nopcache.interned_strings_buffer=8\nopcache.max_accelerated_files=10000\nopcache.memory_consumption=128\nopcache.save_comments=1\nopcache.revalidate_freq=1</code></pre>",
						type: OC.SetupChecks.MESSAGE_TYPE_INFO
					}]);
				done();
			});
