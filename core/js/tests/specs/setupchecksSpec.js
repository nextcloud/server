...
describe('OC.SetupChecks tests', function() {
	...
	describe('checkGeneric', function() {
		...
		it('should return all errors if all headers are missing', function(done) {
			...
			async.done(function( data, s, x ){
				expect(data).toEqual([
				{
					...
					msg: 'The "X-XSS-Protection" HTTP header doesn\'t contain "0". This is a potential security or privacy risk, as it is recommended to adjust this setting accordingly.',
					type: OC.SetupChecks.MESSAGE_TYPE_WARNING
				}, {
		...
		describe('check X-XSS-Protection header', function() {
			it('should return no message if X-XSS-Protection is set to 0', function(done) {
				protocolStub.returns('https');
