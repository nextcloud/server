...
describe('Core base tests', function() {
	...
	describe('Link functions', function() {
		...
		const TESTAPP_ROOT = OC.getRootPath() + '/appsx/testapp'
	...
	describe('Parse query string', function() {
		it('Parses query string from full URL', function() {
			const query = OC.parseQueryString('http://localhost/stuff.php?q=a&b=x')
			expect(query).toEqual({ q: 'a', b: 'x' })
		})
		it('Parses query string from query part alone', function() {
			const query = OC.parseQueryString('q=a&b=x')
			expect(query).toEqual({ q: 'a', b: 'x' })
		})
		it('Returns null hash when empty query', function() {
			const query = OC.parseQueryString('')
		...
		it('Decodes regular query strings', function() {
			const query = OC.parseQueryString('a=abc&b=def')
			expect(query).toEqual({
				a: 'abc',
				b: 'def',
			})
		...
		it('Includes extra equal signs in value', function() {
			const query = OC.parseQueryString('u=a=x&q=a=b')
			expect(query).toEqual({ u: 'a=x', q: 'a=b' })
		})
		it('Decodes plus as space', function() {
			const query = OC.parseQueryString('space+key=space+value')
			expect(query).toEqual({ 'space key': 'space value' })
		})
		it('Decodes special characters', function() {
			let query = OC.parseQueryString('unicode=%E6%B1%89%E5%AD%97')
		...
		it('Decodes empty values', function() {
			const query = OC.parseQueryString('keywithemptystring=&keywithnostring')
			expect(query).toEqual({
				keywithemptystring: '',
		...
		it('Does not interpret data types', function() {
			const query = OC.parseQueryString('booleanfalse=false&booleantrue=true&number=123')
			expect(query).toEqual({
				booleanfalse: 'false',
				booleantrue: 'true',
				number: '123',
	...
	describe('naturalSortCompare', function() {
		...
		const cit = window.isPhantom ? xit : it
