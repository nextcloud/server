OC.L10N.register(
    ...
    "Missing primary key on table \"{tableName}\"." : "Thiếu khóa chính trên bảng \"{tableName}\".",
    ...
    "Missing optional column \"{columnName}\" in table \"{tableName}\"." : "Thiếu cột tùy chọn \"{columnName}\" trong bảng \"{tableName}\".",
    ...
    "Module php-imagick in this instance has no SVG support. For better compatibility it is recommended to install it." : "Mô-đun php-imagick trong trường hợp này không có hỗ trợ SVG. Để tương thích tốt hơn, nên cài đặt nó.",
    "Some columns in the database are missing a conversion to big int. Due to the fact that changing column types on big tables could take some time they were not changed automatically. By running \"occ db:convert-filecache-bigint\" those pending changes could be applied manually. This operation needs to be made while the instance is offline. For further details read {linkstart}the documentation page about this ↗{linkend}." : "Một số cột trong cơ sở dữ liệu bị thiếu chuyển đổi thành số nguyên lớn. Do thực tế là việc thay đổi loại cột trên các bảng lớn có thể mất một thời gian, chúng không được thay đổi tự động. Bằng cách chạy \"occ db:convert-filecache-bigint\", những thay đổi đang chờ xử lý đó có thể được áp dụng thủ công. Thao tác này cần được thực hiện trong khi phiên bản ngoại tuyến. Để biết thêm chi tiết, hãy đọc {linkstart} trang tài liệu về {linkend} này ↗.",
    "SQLite is currently being used as the backend database. For larger installations we recommend that you switch to a different database backend." : "SQLite hiện đang được sử dụng làm cơ sở dữ liệu phụ trợ. Đối với các cài đặt lớn hơn, chúng tôi khuyên bạn nên chuyển sang một backend cơ sở dữ liệu khác.",
    "This is particularly recommended when using the desktop client for file synchronisation." : "Điều này đặc biệt được khuyến nghị khi sử dụng ứng dụng khách trên máy tính để bàn để đồng bộ hóa tệp.",
