OC.L10N.register(
    "core",
    {
    "Please select a file." : "Prašome pasirinkti failą.",
    "File is too big" : "Failas yra per didelis",
    "The selected file is not an image." : "Pasirinktas failas nėra paveikslas.",
    "The selected file cannot be read." : "Nepavyksta perskaityti pasirinkto failo.",
    "The file was uploaded" : "Failas buvo išsiųstas",
    "The uploaded file exceeds the upload_max_filesize directive in php.ini" : "Išsiųstas failas viršija upload_max_filesize direktyvą, esančią faile php.ini",
    "The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form" : "Išsiųstas failas viršija MAX_FILE_SIZE direktyvą, kuri buvo nurodyta HTML formoje",
    "The file was only partially uploaded" : "Failas buvo tik dalinai išsiųstas",
    "No file was uploaded" : "Nebuvo išsiųstas joks failas",
    "Missing a temporary folder" : "Trūksta laikinojo aplanko",
    "Could not write file to disk" : "Nepavyko rašyti failą į diską",
    "A PHP extension stopped the file upload" : "PHP plėtinys sustabdė failo išsiuntimą",
    "Invalid file provided" : "Pateiktas neteisingas failas",
    "No image or file provided" : "Nenurodytas paveikslas ar failas",
    "Unknown filetype" : "Nežinomas failo tipas",
    "An error occurred. Please contact your admin." : "Įvyko klaida. Susisiekite su savo administratoriumi.",
    "Invalid image" : "Netinkamas paveikslas",
    "State token does not match" : "Būsenos žymuo nesutampa",
    "Invalid app password" : "Neteisingas programėlės slaptažodis",
    "Could not complete login" : "Nepavyko užbaigti prisijungimo",
    "State token missing" : "Trūksta būsenos žymens",
    "Your login token is invalid or has expired" : "Jūsų žymuo yra neteisingas arba pasibaigė jo galiojimo laikas",
    "Please use original client" : "Naudokite autentišką kliento programą",
    "This community release of Nextcloud is unsupported and push notifications are limited." : "Ši „Nextcloud\" bendruomenės versija nepalaikoma, o pranešimai yra riboti.",
    "Login" : "Prisijungti",
    "Unsupported email length (>255)" : "Nepalaikomas el. pašto žinutės ilgis (>255)",
    "Password reset is disabled" : "Slaptažodžio atstatymas išjungtas",
    "Could not reset password because the token is expired" : "Nepavyko atstatyti slaptažodžio, nes žymuo nebegalioja",
    "Could not reset password because the token is invalid" : "Nepavyko atstatyti slaptažodžio, nes žymuo yra neteisingas",
    "Password is too long. Maximum allowed length is 469 characters." : "Slaptažodis per ilgas. Didžiausias leistinas ilgis yra 469 simboliai.",
    "%s password reset" : "%s slaptažodžio atstatymas",
    "Password reset" : "Slaptažodžio atstatymas",
    "Click the following button to reset your password. If you have not requested the password reset, then ignore this email." : "Paspauskite mygtuką slaptažodžio atkūrimui. Jei slaptažodžio atkūrimas nėra reikalingas, ignoruokite šį laišką.",
    "Click the following link to reset your password. If you have not requested the password reset, then ignore this email." : "Paspauskite nuorodą slaptažodžio atkūrimui. Jei slaptažodžio atkūrimas nėra reikalingas, ignoruokite šį laišką.",
    "Reset your password" : "Atkurkite savo slaptažodį",
    "The given provider is not available" : "Nurodytas tiekėjas nepasiekiamas",
    "Task not found" : "Užduotis nerasta",
    "Internal error" : "Vidinė klaida",
    "Not found" : "Nerasta",
    "Node is locked" : "Mazgas yra užrakintas",
    "Bad request" : "Bloga užklausa",
    "Input text is too long" : "Įvesties tekstas per ilgas",
    "Requested task type does not exist" : "Prašomos užduoties tipas neegzistuoja",
    "Necessary language model provider is not available" : "Reikiamas kalbos modelio tiekėjas nepasiekiamas",
    "Cannot generate more than 12 images" : "Negalima sugeneruoti daugiau nei 12 paveikslėlių",
    "Cannot generate less than 1 image" : "Negalima sugeneruoti mažiau nei 1 paveikslėlį.",
    "No text to image provider is available" : "Nėra pasiekiamo teksto į vaizdą konvertavimo tiekėjo",
    "Image not found" : "Paveikslėlis nerastas",
    "No translation provider available" : "Nėra pasiekiamo vertimo tiekėjo",
    "Could not detect language" : "Nepavyko aptikti kalbos",
    "Unable to translate" : "Nepavyko išversti",
    "No valid filters provided" : "Nepateikta jokių tinkamų filtrų",
    "Preparing update" : "Ruošiamas atnaujinimas",
    "Already up to date" : "Naudojama naujausia versija",
    "Please use the command line updater because updating via browser is disabled in your config.php." : "Naudokite komandų eilutės atnaujinimo programą, nes atnaujinimas per naršyklę yra išjungtas config.php faile.",
    "[%d / %d]: %s" : "[%d / %d]: %s",
    "Turned on maintenance mode" : "Įjungta techninės priežiūros veiksena",
    "Turned off maintenance mode" : "Išjungta techninės priežiūros veiksena",
    "Maintenance mode is kept active" : "Techninės priežiūros veiksena yra aktyvi",
    "Updating database schema" : "Atnaujinama duomenų bazės struktūra",
    "Updated database" : "Atnaujinta duomenų bazė",
    "Update app \"%s\" from App Store" : "Atnaujinkite \"%s\" programėlę iš „App Store“. ",
    "Checking whether the database schema for %s can be updated (this can take a long time depending on the database size)" : "Tikrinama ar %s duomenų bazės struktūra gali būti atnaujinta (priklausomai nuo duomenų bazės dydžio, tai gali ilgai užtrukti)",
    "Updated \"%1$s\" to %2$s" : "Atnaujinta „%1$s“ į %2$s",
    "Set log level to debug" : "Žurnalo išvesties lygis nustatytas į derinimo",
    "Reset log level" : "Atstatytas registravimo lygis",
    "Starting code integrity check" : "Pradedama kodo vientisumo patikra",
    "Finished code integrity check" : "Kodo vientisumo patikra užbaigta",
    "%s (incompatible)" : "%s (nesuderinama)",
    "The following apps have been disabled: %s" : "Buvo išjungtos šios programėlės: %s",
    "Repair step:" : "Pataisymo žingsnis:",
    "Repair info:" : "Pataisymo informacija:",
    "Repair warning:" : "Pataisymo įspėjimas:",
    "Repair error:" : "Pataisymo klaida:",
    "Log out" : "Atsijungti",
    "Sharing is not allowed." : "Bendrinimas neleidžiamas.",
    "Sharing is not allowed for you." : "Bendrinimas jums neleidžiamas.",
    "Sharing is only allowed with group members." : "Bendrinti leidžiama tik su grupės nariais",
    "Sharing is only allowed within your own groups." : "Bendrinti leidžiama tik savo grupėse.",
    "Group sharing is not allowed." : "Bendrinimas su grupėmis neleidžiamas.",
    "Public link sharing is not allowed." : "Viešas nuorodų bendrinimas neleidžiamas",
    "Sharing to remote users is not allowed." : "Bendrinti su nuotoliniais vartotojais draudžiama.",
    "Sharing to remote groups is not allowed." : "Bendrinti su nuotolinėmis grupėmis draudžiama.",
    "Nextcloud Server" : "„Nextcloud“ serveris",
    "Some of your link shares have been removed" : "Kai kurie jūsų bendrinami saitai buvo pašalinti",
    "Due to a security bug we had to remove some of your link shares. Please see the link for more information." : "Dėl saugumo klaidos teko pašalinti kai kurias jūsų bendrinamas nuorodas. Daugiau informacijos rasite nuorodoje.",
    "The account limit of this instance is reached." : "Šiame egzemplioriuje yra pasiektas paskyrų limitas.",
    "Enter your subscription key in the support app in order to increase the account limit. This does also grant you all additional benefits that Nextcloud Enterprise offers and is highly recommended for the operation in companies." : "Įveskite savo prenumeratos prieigos raktą palaikymo programėlėje, kad padidintumėte paskyros limitą. Tai taip pat suteikia visas papildomas „Nextcloud Enterprise\" teikiamas naudas ir yra labai rekomenduojama įmonėms.",
    "Learn more ↗" : "Sužinokite daugiau ↗",
    "Can edit" : "Gali redaguoti",
    "Share with others" : "Bendrinti su kitais",
    "Can view" : "Gali matyti",
    "Expiration date" : "Galiojimo pabaigos data",
    "Your administrator has enforced a %d days expiration policy." : "Jūsų administratorius nustatė galiojimo terminą, kuris baigiasi po %d dienų.",
    "Label" : "Etiketė",
    "Note to recipients" : "Pastaba gavėjams",
    "Password" : "Slaptažodis",
    "Your administrator has enforced a password protection." : "Jūsų administratorius įjungė slaptažodžio apsaugą.",
    "Email" : "El. paštas",
    "Group" : "Grupė",
    "Team" : "Komanda",
    "Public link" : "Viešoji nuoroda",
    "User" : "Vartotojas",
    "Windows Command Script" : "„Windows\" komandų tekstas",
    "Electronic book document" : "Elektroninės knygos dokumentas",
    "TrueType Font Collection" : "„TrueType“ šriftų rinkinys",
    "Web Open Font Format" : "Žiniatinklio atvirasis šrifto formatas (Web OFT)",
    "GPX geographic data" : "GPX geografiniai duomenys",
    "Gzip archive" : "Gzip archyvas",
    "Adobe Illustrator document" : "„Adobe Illustrator“ dokumentas",
    "Java source code" : " „Java\" šaltinio kodas",
    "JavaScript source code" : "„JavaScript\" šaltinio kodas",
    "JSON document" : "JSON dokumentas",
    "Microsoft Access database" : "„Microsoft Access“ duomenų bazė",
    "Microsoft OneNote document" : "„Microsoft OneNote“ dokumentas",
    "Microsoft Word document" : "„Microsoft Word“ dokumentas",
    "Unknown" : "Nežinomas",
    "PDF document" : "PDF dokumentas",
    "PostScript document" : "„PostScript“ dokumentas",
    "RSS summary" : "RSS santrauka",
    "Android package" : "„Android“ paketas",
    "KML geographic data" : "KML geografiniai duomenys",
    "KML geographic compressed data" : " Suglaudiniti KML geografiniai duomenys",
    "Lotus Word Pro document" : "„Lotus Word Pro“ dokumentas",
    "Excel spreadsheet" : "„Excel“ skaičiuoklė",
    "Excel add-in" : "„Excel“ papildinys",
    "Excel 2007 binary spreadsheet" : "„Excel 2007“ dvejetainė skaičiuoklė",
    "Excel spreadsheet template" : "„Excel“ skaičiuoklės šablonas",
    "Outlook Message" : "„Outlook“ laiškas",
    "PowerPoint presentation" : "„PowerPoint“ pateiktis",
    "PowerPoint add-in" : "„PowerPoint“ papildinys",
    "PowerPoint presentation template" : "„PowerPoint“ pateikties šablonas",
    "Word document" : "„Word“ dokumentas",
    "ODF formula" : "ODF formulė",
    "ODG drawing" : "ODG grafikos dokumentas",
    "ODG drawing (Flat XML)" : "ODG grafikos dokumentas (Plokščiasis XML)",
    "ODG template" : "ODG šablonas",
    "ODP presentation" : "ODP pateiktis",
    "ODP presentation (Flat XML)" : "ODP pateiktis (Plokščiasis XML)",
    "ODP template" : "ODP šablonas",
    "ODS spreadsheet" : "ODS skaičiuoklė",
    "ODS spreadsheet (Flat XML)" : "ODS skaičiuoklė (Plokščiasis XML)",
    "ODS template" : "ODS šablonas",
    "ODT document" : "ODT dokumentas",
    "ODT document (Flat XML)" : "ODT dokumentas (Plokščiasis XML)",
    "ODT template" : "ODT šablonas",
    "PowerPoint 2007 presentation" : "„PowerPoint 2007“ pateiktis",
    "PowerPoint 2007 show" : "„PowerPoint 2007“ demonstracija",
    "PowerPoint 2007 presentation template" : "„PowerPoint 2007“ pateikties šablonas",
    "Excel 2007 spreadsheet" : "„Excel 2007“ skaičiuoklė",
    "Excel 2007 spreadsheet template" : "„Excel 2007“ skaičiuoklės šablonas",
    "Word 2007 document" : "„Word 2007“ dokumentas",
    "Word 2007 document template" : "„Word 2007“ dokumento šablonas",
    "Microsoft Visio document" : "„Microsoft Visio“ dokumentas",
    "WordPerfect document" : "„WordPerfect“ dokumentas",
    "7-zip archive" : "7-zip archyvas",
    "Blender scene" : "\"Blender\" vaizdas",
    "Bzip2 archive" : "Bzip2 archyvas",
    "Debian package" : "„Debian“ paketas",
    "FictionBook document" : "„FictionBook“ dokumentas",
    "Unknown font" : "Nežinomas šriftas",
    "Krita document" : "„Krita“ dokumentas",
    "Mobipocket e-book" : "„Mobipocket“ el. knyga",
    "Windows Installer package" : "„Windows“ diegimo programos paketas",
    "Perl script" : "Perl scenarijus",
    "PHP script" : "PHP scenarijus",
    "Tar archive" : "Tar archyvas",
    "XML document" : "XML dokumentas",
    "YAML document" : "YAML dokumentas",
    "Zip archive" : "Zip archyvas",
    "Zstandard archive" : "Zstandard archyvas",
    "AAC audio" : "AAC garso įrašas",
    "FLAC audio" : "FLAC garso įrašas",
    "MPEG-4 audio" : "MPEG-4 garso įrašas",
    "MP3 audio" : "MP3 garso įrašas",
    "Ogg audio" : "Ogg garso įrašas",
    "RIFF/WAVe standard Audio" : "RIFF/WAVe standarto garso įrašas",
    "WebM audio" : "WebM garso įrašas",
    "MP3 ShoutCast playlist" : "MP3 „ShoutCast“ grojaraštis",
    "Windows BMP image" : "„Windows“ BMP paveikslėlis",
    "Better Portable Graphics image" : "\"Better Portable Graphics\" paveikslėlis",
    "EMF image" : "EMF paveikslėlis",
    "GIF image" : "GIF paveikslėlis",
    "HEIC image" : "HEIC paveikslėlis",
    "HEIF image" : "HEIF paveikslėlis",
    "JPEG-2000 JP2 image" : "\"JPEG-2000 JP2\" paveikslėlis",
    "JPEG image" : "JPEG paveikslėlis",
    "PNG image" : "PNG paveikslėlis",
    "SVG image" : "SVG paveikslėlis",
    "Truevision Targa image" : "\"Truevision Targa\" paveikslėlis",
    "TIFF image" : "TIFF paveikslėlis",
    "WebP image" : "WebP paveikslėlis",
    "Digital raw image" : "Skaitmeninis neapdorotas vaizdas",
    "Windows Icon" : "„Windows“ piktograma",
    "Email message" : "Elektroninis laiškas",
    "VCS/ICS calendar" : "VCS/ICS kalendorius",
    "CSS stylesheet" : "CSS stilių aprašas",
    "CSV document" : "CSV dokumentas",
    "TSV document" : "TSV dokumentas",
    "HTML document" : "HTML dokumentas",
    "Markdown document" : "„Markdown“ dokumentas",
    "Org-mode file" : "„Org-mode“ failas",
    "Plain text document" : "Grynojo teksto dokumentas",
    "Rich Text document" : "Raiškiojo teksto dokumentas",
    "Electronic business card" : "Elektroninė vizitinė kortelė",
    "C++ source code" : "C++ pradinis kodas",
    "LDIF address book" : "LDIF adresų knyga",
    "NFO document" : "NFO dokumentas",
    "PHP source" : "PHP šaltinis",
    "Python script" : "Python scenarijus",
    "ReStructuredText document" : "„ReStructuredText“ dokumentas",
    "3GPP multimedia file" : "3GPP multimedijos failas",
    "MPEG video" : "MPEG vaizdo įrašas",
    "DV video" : "DV vaizdo įrašas",
    "MPEG-2 transport stream" : "MPEG-2 perdavimo srautas",
    "MPEG-4 video" : "MPEG-4 vaizdo įrašas",
    "Ogg video" : "Ogg vaizdo įrašas",
    "QuickTime video" : "QuickTime vaizdo įrašas",
    "WebM video" : "WebM vaizdo įrašas",
    "Flash video" : "Flash vaizdo įrašas",
    "Matroska video" : "Matroska vaizdo įrašas",
    "Windows Media video" : "Windows Media vaizdo įrašas",
    "AVI video" : "AVI vaizdo įrašas",
    "Show QR code for mobile app login" : "Rodyti QR kodą prisijungimui prie mobiliosios programėlės",
    "Done" : "Atlikta",
    "Scan QR code to log in" : "Nuskenuokite QR kodą norėdami prisijungti",
    "Use {productName} mobile client you want to connect to scan the code" : "Norėdami nuskaityti kodą, naudokite {productName} mobilųjį klientą, prie kurio norite prisijungti",
    "Code will expire {timeCountdown} or after use" : "Kodas baigs galioti jį panaudojus arba {timeCountdown}",
    "More apps" : "Daugiau programėlių",
    "App store" : "Programėlių parduotuvė",
    "Settings" : "Nustatymai",
    "Profile" : "Profilis",
    "Open apps menu, currently in {app}" : "Atidaryti programėlių meniu, šiuo metu {app}",
    "Open apps menu" : "Atidarykite programėlių meniu",
    "Applications" : "Programėlės",
    "Apps" : "Programėlės",
    "More actions" : "Daugiau veiksmų",
    "Actions" : "Veiksmai",
    "_{count} notification_::_{count} notifications_" : ["{count} pranešimas","{count} pranešimai","{count} pranešimų","{count} pranešimas"],
    "No" : "Ne",
    "Yes" : "Taip",
    "App password" : "Programėlės slaptažodis",
    "Grant access" : "Suteikti prieigą",
    "The remote URL must include the user." : "Nuotoliniame URL turi būti nurodytas vartotojas.",
    "Invalid remote URL." : "Neteisingas nuotolinis URL.",
    "Failed to add the public link to your Nextcloud" : "Nepavyko pridėti viešosios nuorodos į jūsų „Nextcloud“",
    "Federated user" : "Federacinis vartotojas",
    "user@your-nextcloud.org" : "vartotojas@jūsų-nextcloud.org",
    "Create share" : "Sukurti bendrinimą",
    "Direct link copied" : "Tiesioginė nuoroda nukopijuota",
    "Please copy the link manually:" : "Nukopijuokite nuorodą rankiniu būdu:",
    "Custom date range" : "Pasirinktinis datų rėžis",
    "Pick start date" : "Pasirinkti pradžios datą",
    "Pick end date" : "Pasirinkti pabaigos datą",
    "Search in date range" : "Paieška datų rėžyje",
    "Remove filter: {name}" : "Pašalinti filtrą: {name}",
    "Apps, files, messages, and more" : "Programėlės, failai, pranešimai ir kita",
    "Filters" : "Filtrai",
    "Clear search" : "Išvalyti paiešką",
    "Close search" : "Uždaryti paiešką",
    "Start typing to search" : "Rašykite norėdami atlikti paiešką",
    "_Minimum search length is %n character_::_Minimum search length is %n characters_" : ["Minimalus paieškos simbolių skaičius yra %n","Minimalus paieškos simbolių skaičius yra %n","Minimalus paieškos simbolių skaičius yra %n","Minimalus paieškos simbolių skaičius yra %n"],
    "No matching results" : "Nėra atitinkančių rezultatų",
    "Less from connected services" : "Mažiau iš prijungtų paslaugų",
    "More from connected services" : "Daugiau iš prijungtų paslaugų",
    "Searching …" : "Ieškoma…",
    "_Showing %n result from {name}_::_Showing %n results from {name}_" : ["Rodomas %n rezultatas iš {name}","Rodomi %n rezultatai iš {name}","Rodoma %n rezultatų iš {name}","Rodomi %n rezultatai iš {name}"],
    "_%n result_::_%n results_" : ["%n rezultatas","%n rezultatai","%n rezultatų","%n rezultatų"],
    "Today" : "Šiandien",
    "Last 7 days" : "Paskutinės 7 dienos",
    "Last 30 days" : "Paskutinės 30 dienų",
    "This year" : "Šie metai",
    "Last year" : "Praėję metai",
    "Between {startDate} and {endDate}" : "Tarp {startDate} ir {endDate}",
    "Type" : "Tipas",
    "Date" : "Data",
    "Search people" : "Žmonių paieška",
    "People" : "Žmonės",
    "Results" : "Rezultatai",
    "Back to all results" : "Grįžti prie visų rezultatų",
    "Back" : "Atgal",
    "Partial matches" : "Daliniai atitikimai",
    "More from {name}" : "Daugiau iš {name}",
    "Load more results" : "Įkelti daugiau rezultatų",
    "Search in" : "Ieškoti",
    "Log in" : "Prisijungti",
    "Logging in …" : "Prisijungiama…",
    "Log in to {productName}" : "Prisijunkite prie {productName}",
    "Wrong login or password." : "Neteisingas prisijungimas ar slaptažodis.",
    "This account is disabled" : "Ši paskyra yra išjungta",
    "Too many failed login attempts from your location. Try again in 30 seconds." : "Per daug nepavykusių prisijungimo bandymų iš jūsų vietos. Bandykite dar kartą po 30 sekundžių.",
    "Account name or email" : "Paskyros vardas ar el. paštas",
    "Account name" : "Paskyros vardas",
    "Server side authentication failed!" : "Autentikacija serveryje nepavyko!",
    "Please contact your administrator." : "Susisiekite su administratoriumi.",
    "Session error" : "Seanso klaida",
    "It appears your session token has expired, please refresh the page and try again." : "Panašu, kad jūsų sesijos žymuo nebegalioja, atnaujinkite puslapį ir bandykite dar kartą.",
    "An internal error occurred." : "Įvyko vidinė klaida.",
    "Please try again or contact your administrator." : "Bandykite dar kartą arba susisiekite su administratoriumi.",
    "Remember me" : "Prisiminti mane",
    "Log in with a device" : "Prisijungti naudojant įrenginį",
    "Login or email" : "Prisijungimas ar el. paštas",
    "Your account is not setup for passwordless login." : "Jūsų paskyra nėra nustatyta prisijungimui be slaptažodžio.",
    "Your connection is not secure" : "Jūsų ryšys nėra saugus",
    "Passwordless authentication is only available over a secure connection." : "Tapatybės nustatymas be slaptažodžio yra prieinamas tik per saugų ryšį.",
    "Browser not supported" : "Naršyklė nepalaikoma",
    "Passwordless authentication is not supported in your browser." : "Tapatybės nustatymas be slaptažodžio nėra palaikomas jūsų naršyklėje.",
    "Reset password" : "Atstatyti slaptažodį",
    "Back to login" : "Grįžti prie prisijungimo",
    "If this account exists, a password reset message has been sent to its email address. If you do not receive it, verify your email address and/or Login, check your spam/junk folders or ask your local administration for help." : "Jei ši paskyra yra, laiškas su slaptažodžio atstatymo instrukcija buvo išsiųstas į jos el. pašto adresą. Jeigu jo negavote, patikrinkite el. pašto adresą ir (arba) prisijungimą, patikrinkite brukalo/šlamšto pašto aplankus arba paprašykite vietinio administratoriaus pagalbos.",
    "Couldn't send reset email. Please contact your administrator." : "Nepavyko išsiųsti atstatymo el. laiško. Susisiekite su savo administratoriumi.",
    "Password cannot be changed. Please contact your administrator." : "Slaptažodis negali būti pakeistas. Susisiekite su administratoriumi.",
    "New password" : "Naujas slaptažodis",
    "Your files are encrypted. There will be no way to get your data back after your password is reset. If you are not sure what to do, please contact your administrator before you continue. Do you really want to continue?" : "Jūsų failai yra užšifruoti. Atstačius slaptažodį, niekaip nebegalėsite susigrąžinti duomenų. Jei nesate tikri, ką daryti, prieš tęsdami susisiekite su administratoriumi. Ar tikrai norite tęsti?",
    "I know what I'm doing" : "Aš žinau, ką darau",
    "Resetting password" : "Slaptažodžio nustatymas iš naujo",
    "Schedule work & meetings, synced with all your devices." : "Planuokite darbą ir susitikimus, sinchronizuodami juos su visais savo įrenginiais.",
    "Keep your colleagues and friends in one place without leaking their private info." : "Saugokite savo kolegas bei draugus vienoje vietoje, neatskleisdami jų asmeninės informacjios.",
    "Simple email app nicely integrated with Files, Contacts and Calendar." : "Paprasta el. pašto programėlė, tvarkingai integruota su failais, adresatais ir kalendoriumi.",
    "Chatting, video calls, screen sharing, online meetings and web conferencing – in your browser and with mobile apps." : "Pokalbiai, vaizdo skambučiai, ekrano bendrinimas, internetiniai susitikimai ir žiniatinklio konferencijos – naršyklėje ir mobiliosiose programėlėse.",
    "Collaborative documents, spreadsheets and presentations, built on Collabora Online." : "Bendradarbiavimo dokumentai, skaičiuoklės ir pristatymai, sukurti naudojant „Collabora Online“.",
    "Distraction free note taking app." : "Užrašų darymo programėlė be blaškymosi.",
    "Password confirmation was aborted" : "Slaptažodžio patvirtinimas buvo nutrauktas",
    "App download or installation failed" : "Programėlės atsiuntimas ar įdiegimas patyrė nesėkmę",
    "Recommended apps" : "Rekomenduojamos programėlės",
    "Loading apps …" : "Įkeliamos programėlės…",
    "Could not fetch list of apps from the App Store." : "Nepavyko gauti programėlių sąrašo iš „App Store“.",
    "App already installed" : "Programėlė jau įdiegta",
    "Cannot install this app because it is not compatible" : "Nepavyksta įdiegti šios programėlės, nes ji yra nesuderinama",
    "Cannot install this app" : "Nepavyksta įdiegti šios programėlės",
    "Skip" : "Praleisti",
    "Installing apps …" : "Įdiegiamos programėlės…",
    "Install recommended apps" : "Įdiegti rekomenduojamas programėles",
    "Online" : "Prisijungęs",
    "Away" : "Atsitraukęs",
    "Busy" : "Užimtas",
    "Do not disturb" : "Netrukdyti",
    "Mute all notifications" : "Išjungti visus pranešimus",
    "Invisible" : "Nematomas",
    "Appear offline" : "Atrodyti atsijungusiu",
    "Avatar of {displayName}" : "{displayName} avataras",
    "Settings menu" : "Nustatymų meniu",
    "Loading your contacts …" : "Įkeliami adresatai…",
    "Looking for {term} …" : "Ieškoma {term}…",
    "Search contacts" : "Ieškoti adresatų",
    "Filter by team" : "Filtruoti pagal komandą",
    "All teams" : "Visos komandos",
    "Search contacts in team {team}" : "Ieškoti adresatų komandoje {team}",
    "Search contacts …" : "Ieškoti adresatų…",
    "Reset search" : "Atstatyti paiešką",
    "Could not load your contacts" : "Nepavyko įkelti adresatų",
    "No contacts found" : "Kontaktų nerasta",
    "Contacts list" : "Adresatų sąrašas",
    "Show all contacts" : "Rodyti visus adresatus",
    "Install the Contacts app" : "Įdiegti adresatų programėlę",
    "Search" : "Ieškoti",
    "No results for {query}" : "{query} negrąžino jokių rezultatų",
    "Press Enter to start searching" : "Norėdami pradėti paiešką paspauskite įvedimo (Enter) klavišą",
    "_Please enter {minSearchLength} character or more to search_::_Please enter {minSearchLength} characters or more to search_" : ["Norėdami atlikti paiešką, įveskite {minSearchLength} ar daugiau simbolių","Norėdami atlikti paiešką, įveskite {minSearchLength} ar daugiau simbolių","Norėdami atlikti paiešką, įveskite {minSearchLength} ar daugiau simbolių","Norėdami atlikti paiešką, įveskite {minSearchLength} ar daugiau simbolių"],
    "An error occurred while searching for {type}" : "Ieškant {type}, įvyko klaida",
    "Search starts once you start typing and results may be reached with the arrow keys" : "Paieška prasideda pradėjus rašyti, o rezultatus galima pasiekti rodyklių klavišais",
    "Search for {name} only" : "Ieškoti tik {name}",
    "Loading more results …" : "Įkeliama daugiau rezultatų…",
    "Forgot password?" : "Pamiršote slaptažodį?",
    "Back to login form" : "Grįžti į prisijungimo formą",
    "Login form is disabled." : "Prisijungimo forma išjungta.",
    "The Nextcloud login form is disabled. Use another login option if available or contact your administration." : "„Nextcloud“ prisijungimo forma išjungta. Jei įmanoma, naudokite kitą prisijungimo parinktį arba susisiekite su sistemos administratoriumi.",
    "Connect to your account" : "Prisijunkite prie paskyros",
    "Please log in before granting \"{client}\" access to your {instanceName} account." : "Prieš suteikdami \"{client}\" prieigą prisijunkite prie savo {instanceName} paskyros.",
    "Security warning" : "Saugumo įspėjimas",
    "If you are not trying to set up a new device or app, someone is trying to trick you into granting them access to your data. In this case do not proceed and instead contact your system administrator." : "Jeigu nebandote nusistatyti naujo įrenginio ar programėlės, tuomet kažkas bando jus apgauti, kad suteiktumėte jam prieigą prie savo duomenų. Tokiu atveju netęskite ir, vietoj to, susisiekite su savo sistemos administratoriumi.",
    "Log in using password" : "Prisijungimas naudojant slaptažodį",
    "Alternative log in using app password" : "Alternatyvus prisijungimas naudojant programėlės slaptažodį",
    "Account connected" : "Paskyra prijungta",
    "Your client should now be connected!" : "Dabar, jūsų kliento programa turėtų būti prijungta!",
    "You can close this window." : "Galite užverti šį langą.",
    "Account access" : "Paskyros prieiga",
    "Currently logged in as {userDisplayName} ({userId})." : "Šiuo metu esate prisijungę kaip {userDisplayName} ({userId}).",
    "You are about to grant \"{client}\" access to your {instanceName} account." : "Jūs ketinate suteikti „{client}“ prieigą prie savo {instanceName} paskyros.",
    "User menu" : "Vartotojo meniu",
    "Your guest name: {user}" : "Jūsų kaip svečio vardas yra: {user}",
    "You are currently not identified." : "Šiuo metu nesate identifikuoti.",
    "Set public name" : "Viešai matomo vardo nustatymas",
    "Change public name" : "Viešai matomo vardo keitimas",
    "This share is password-protected" : "Šis bendrinimas yra apsaugotas slaptažodžiu",
    "The password is wrong or expired. Please try again or request a new one." : "Neteisingas arba nebegaliojantis slaptažodis. Bandykite dar kartą arba paprašykite naujo slaptažodžio.",
    "Submit" : "Pateikti",
    "If the email address was correct then you will receive an email with the password." : "Jei el. pašto adresas buvo teisingas, gausite el. laišką su slaptažodžiu.",
    "Please type in your email address to request a temporary password" : "Norėdami paprašyti laikinojo slaptažodžio įrašykite savo elektroninio pašto adresą",
    "Email address" : "El. pašto adresas",
    "Request password" : "Prašyti slaptažodžio",
    "Forgot password" : "Pamiršau slaptažodį",
    "{name} version {version} and above" : "{name}  {version} versija arba naujesnė",
    "This browser is not supported" : "Ši naršyklė nepalaikoma",
    "Your browser is not supported. Please upgrade to a newer version or a supported one." : "Jūsų naršyklė yra nepalaikoma. Atnaujinkite į naujesnę versiją arba naudokite palaikomą naršyklę.",
    "Continue with this unsupported browser" : "Tęsti naudojant šią nepalaikomą naršyklę",
    "Supported versions" : "Palaikomos versijos",
    "The update completed with warnings. Please check the details for more information." : "Atnaujinimas užbaigtas su įspėjimais. Daugiau informacijos rasite išsamesnėje informacijoje.",
    "The update completed successfully." : "Atnaujinimas sėkmingai užbaigtas.",
    "Preparing update…" : "Ruošiamas atnaujinimas…",
    "You will be redirected to {productName} in {count} seconds." : "Po {count} sek. būsite peradresuoti į {productName}.",
    "The update is in progress, leaving this page might interrupt the process in some environments." : "Vykdomas atnaujinimas, palikus šį puslapį atvertą, kai kuriose aplinkose šis procesas gali būti nutrauktas.",
    "App update required" : "Reikalingas programėlės atnaujinimas",
    "{productName} will be updated to version {version}" : "{productName} bus atnaujinta iki {version} versijos.",
    "The theme {oldTheme} has been disabled." : "{oldTheme} tema buvo išjungta.",
    "These incompatible apps will be disabled:" : "Bus išjungtos šios nesuderinamos programėlės:",
    "Incompatible apps" : "Nesuderinamos programėlės",
    "These apps will be updated:" : "Bus atnaujintos šios programėlės:",
    "Apps to update" : "Atnaujintinos programėlės",
    "{app} from {oldVersion} to {version}" : "{app} iš {oldVersion} į {version} versiją.",
    "Please make sure that the database, the config folder and the data folder have been backed up before proceeding." : "Prieš tęsdami, įsitikinkite, kad pasidarėte duomenų bazės, konfigūracijos aplanko bei duomenų aplanko atsarginę kopiją.",
    "To avoid timeouts with larger installations, you can instead run the following command from your installation directory:" : "Naudodami didesnius diegimus ir norėdami išvengti laukimo laiko, vietoj to, savo diegimo kataloge galite paleisti šią komandą:",
    "Start update" : "Pradėti atnaujinimą",
    "Continue to {productName}" : "Tęsti {productName}",
    "Update to {version}" : "Atnaujinti į versiją {version}",
    "Hide details" : "Slėpti išsamesnę informaciją",
    "Show details" : "Rodyti išsamesnę informaciją",
    "Update details" : "Atnaujinti informaciją",
    "Update needed" : "Reikalingas atnaujinimas",
    "Please use the command line updater because you have a big instance with more than 50 accounts." : "Naudokite komandinės eilutės atnaujinimo įrankį, nes turite didelę užklausą su daugiau nei 50 paskyrų.",
    "Documentation" : "Dokumentacija",
    "I know that if I continue doing the update via web UI has the risk, that the request runs into a timeout and could cause data loss, but I have a backup and know how to restore my instance in case of a failure." : "Suprantu, kad atnaujinimui naudodamas vartotojo aplinką, rizikuoju, kad sistemos darbas sutriks ir prisijungę vartotojai gali netekti duomenų, turiu atsarginę duomenų kopiją ir žinau, kaip atstatyti duomenis nesėkmės atveju.",
    "Upgrade via web on my own risk" : "Atnaujinti per interneto naršyklę prisiimant riziką",
    "Password is too weak" : "Per silpnas slaptažodis",
    "Password is weak" : "Silpnas slaptažodis",
    "Password is average" : "Vidutinio stiprumo slaptažodis",
    "Password is strong" : "Stiprus slaptažodis",
    "Password is very strong" : "Labai stiprus slaptažodis",
    "Password is extremely strong" : "Ypatingai stiprus slaptažodis",
    "Unknown password strength" : "Nežinomas slaptažodžio stiprumas",
    "Your data directory and files are probably accessible from the internet because the <code>.htaccess</code> file does not work." : "Jūsų duomenų katalogas ir failai, tikriausiai, pasiekiami iš interneto, nes <code>.htaccess </code> failas neveikia.",
    "For information how to properly configure your server, please {linkStart}see the documentation{linkEnd}" : "Informacijos, kaip tinkamai sukonfigūruoti serverį, ieškokite {linkStart} dokumentacijoje {linkEnd}",
    "Autoconfig file detected" : "Aptiktas Autoconfig failas",
    "The setup form below is pre-filled with the values from the config file." : "Žemiau esanti sąrankos forma yra iš anksto užpildyta reikšmėmis iš konfigūracijos failo.",
    "Create administration account" : "Sukurti administravimo paskyrą",
    "Administration account name" : "Administravimo paskyros vardas",
    "Administration account password" : "Administravimo paskyros slaptažodis",
    "Storage & database" : "Saugykla ir duomenų bazė",
    "Data folder" : "Duomenų aplankas",
    "Database configuration" : "Duomenų bazės konfigūracija",
    "Database type" : "Duomenų bazės tipas",
    "Only {firstAndOnlyDatabase} is available." : "Prieinama tik {firstAndOnlyDatabase}.",
    "Install and activate additional PHP modules to choose other database types." : "Įdiekite ir aktyvuokite papildomus PHP modulius, kad pasirinktumėte kitus duomenų bazės tipus.",
    "For more details check out the documentation." : "Išsamesnei informacijai, žiūrėkite dokumentaciją.",
    "Performance warning" : "Įspėjimas apie našumą",
    "You chose SQLite as database." : "Jūs kaip duomenų bazę pasirinkote SQLite.",
    "SQLite should only be used for minimal and development instances. For production we recommend a different database backend." : "SQLite turėtų būti naudojama tik minimaliems ir plėtojimo egzemplioriams. Darbiniam egzemplioriui rekomenduojame naudoti kitą duomenų bazės vidinę pusę.",
    "If you use clients for file syncing, the use of SQLite is highly discouraged." : "Jei sinchronizavimui naudojate kliento programas, tuomet SQLite naudojimas yra primygtinai nerekomenduojamas.",
    "Database connection" : "Duomenų bazės sąsaja",
    "Database user" : "Duomenų bazės vartotojas",
    "Database password" : "Duomenų bazės slaptažodis",
    "Database name" : "Duomenų bazės pavadinimas",
    "Database tablespace" : "Duomenų bazės loginis saugojimas",
    "Please specify the port number along with the host name (e.g., localhost:5432)." : "Nurodykite prievado numerį kartu su pagrindinio kompiuterio pavadinimu (pvz., localhost:5432).",
    "Database host" : "Duomenų bazės serveris",
    "localhost" : "\"localhost\" - vietinis serveris",
    "Encrypted database connection" : "Šifruotas duomenų bazės ryšys",
    "Supported modes: disable, allow, prefer, require, verify-ca, verify-full." : "Palaikomi režimai: išjungti, leisti, teikti pirmenybę, reikalauti, tikrinti CA, tikrinti visapusiškai.",
    "Encryption mode" : "Šifravimo režimas",
    "Has to be readable by the web server." : "Turi būti skaitomas žiniatinklio serverio.",
    "CA certificate path" : "CA sertifikato kelias",
    "Client certificate path" : "Kliento sertifikato kelias",
    "Client certificate key path" : "Kliento sertifikato rakto kelias",
    "Certificate revocation list path" : "Sertifikatų atšaukimo sąrašo kelias",
    "Do not verify that the server certificate matches the database host" : "Nereikia tikrinti, ar serverio sertifikatas atitinka duomenų bazės serverį",
    "Installing …" : "Įdiegiama…",
    "Install" : "Įdiegti",
    "Need help?" : "Reikia pagalbos?",
    "See the documentation" : "Žiūrėkite dokumentaciją",
    "Search {types} …" : "Paieška {types}",
    "Choose {file}" : "Pasirinkti {file}",
    "Choose" : "Pasirinkti",
    "Copy to {target}" : "Kopijuoti į {target}",
    "Copy" : "Kopijuoti",
    "Move to {target}" : "Perkelti į {target}",
    "Move" : "Perkelti",
    "OK" : "Gerai",
    "Saving …" : "Įrašoma…",
    "seconds ago" : "prieš keletą sekundžių",
    "Add to a project" : "Pridėti į projektą",
    "Rename project" : "Pervadinti projektą",
    "Failed to rename the project" : "Nepavyko pervadinti projekto",
    "Failed to create a project" : "Nepavyko sukurti projekto",
    "Failed to add the item to the project" : "Nepavyko pridėti elemento į projektą",
    "Connect items to a project to make them easier to find" : "Susieti elementus su projektu, kad juos būtų lengviau rasti",
    "Type to search for existing projects" : "Rašykite norėdami ieškoti esamų projektų",
    "Email length is at max (255)" : "El. pašto žinutės maksimalus ilgis (255)",
    "Clipboard not available, please copy manually" : "Iškarpinės funkcija negalima, prašom kopijuoti rankiniu būdu",
    "Personal" : "Asmeniniai",
    "Accounts" : "Paskyros",
    "Admin" : "Administravimas",
    "Help" : "Pagalba",
    "Access forbidden" : "Prieiga uždrausta",
    "You are not allowed to access this page." : "Jums neleidžiama prieiga prie šio puslapio.",
    "Back to %s" : "Atgal į %s",
    "Page not found" : "Puslapis nerastas",
    "The page could not be found on the server or you may not be allowed to view it." : "Puslapio nepavyko rasti serveryje arba jums gali būti neleidžiama jo peržiūrėti.",
    "Too many requests" : "Per daug užklausų",
    "There were too many requests from your network. Retry later or contact your administrator if this is an error." : "Iš jūsų tinklo buvo per daug užklausų. Bandykite dar kartą vėliau arba, jeigu tai klaida, susisiekite su savo administratoriumi.",
    "Error" : "Klaida",
    "Internal Server Error" : "Vidinė serverio klaida",
    "The server was unable to complete your request." : "Serveriui nepavyko įvykdyti jūsų užklausos.",
    "If this happens again, please send the technical details below to the server administrator." : "Jei tai pasikartos, prašome nusiųsti serverio administratoriui žemiau esančią techninę informaciją.",
    "More details can be found in the server log." : "Išsamesnė informacija yra prieinama serverio žurnale.",
    "For more details see the documentation ↗." : "Išsamesnei informacijai žiūrėkite dokumentaciją ↗.",
    "Technical details" : "Techniniai duomenys",
    "Remote Address: %s" : "Nuotolinis adresas: %s",
    "Request ID: %s" : "Užklausos ID: %s",
    "Type: %s" : "Tipas: %s",
    "Code: %s" : "Kodas: %s",
    "Message: %s" : "Žinutė: %s",
    "File: %s" : "Failas: %s",
    "Line: %s" : "Eilutė: %s",
    "Trace" : "Sekti",
    "It looks like you are trying to reinstall your Nextcloud. However the file CAN_INSTALL is missing from your config directory. Please create the file CAN_INSTALL in your config folder to continue." : "Panašu, kad bandote iš naujo įdiegti „Nextcloud“. Tačiau konfigūracijos kataloge trūksta failo CAN_INSTALL. Norėdami tęsti, sukurkite failą CAN_INSTALL konfigūracijos aplanke.",
    "Could not remove CAN_INSTALL from the config folder. Please remove this file manually." : "Nepavyko iš konfigūracijos aplanko pašalinti failo CAN_INSTALL. Pašalinkite šį failą rankiniu būdu.",
    "This application requires JavaScript for correct operation. Please {linkstart}enable JavaScript{linkend} and reload the page." : "Sistemos veikimui reikalingas JavaScript palaikymas. Prašome {linkstart}įjungti JavaScript palaikymą{linkend} ir atnaujinti puslapį.",
    "Skip to main content" : "Pereiti į pagrindinį turinį",
    "Skip to navigation of app" : "Pereiti į programėlės naršymą",
    "Go to %s" : "Pereiti į %s",
    "Get your own free account" : "Gaukite nemokamą asmeninę paskyrą",
    "Previous" : "Ankstesnis",
    "Two-factor authentication" : "Dviejų faktorių tapatybės nustatymas",
    "Enhanced security is enabled for your account. Choose a second factor for authentication:" : "Jūsų paskyroje įjungtas sustiprintas saugumas. Pasirinkite antrą autentifikavimo veiksnį:",
    "Could not load at least one of your enabled two-factor auth methods. Please contact your admin." : "Nepavyko įkelti bent vieno iš jūsų įjungtų dviejų faktorių tapatybės nustatymo metodų. Susisiekite su savo administratoriumi.",
    "Two-factor authentication is enforced but has not been configured on your account. Contact your admin for assistance." : "Dviejų faktorių tapatybės nustatymas yra priverstinis, tačiau nebuvo sukonfigūruotas jūsų paskyroje. Susisiekite su savo administratoriumi dėl pagalbos. ",
    "Two-factor authentication is enforced but has not been configured on your account. Please continue to setup two-factor authentication." : "Dviejų faktorių tapatybės nustatymas yra priverstinis, tačiau nebuvo sukonfigūruotas jūsų paskyroje. Tęskite, norėdami nustatyti dviejų faktorių tapatybės nustatymą.",
    "Set up two-factor authentication" : "Nustatyti dviejų faktorių autentifikavimą",
    "Two-factor authentication is enforced but has not been configured on your account. Use one of your backup codes to log in or contact your admin for assistance." : "Dviejų faktorių tapatybės nustatymas yra priverstinis, tačiau nebuvo sukonfigūruotas jūsų paskyroje. Norėdami prisijungti, naudokite vieną iš savo atsarginių kodų arba susisiekite su savo administratoriumi dėl pagalbos. ",
    "Use backup code" : "Naudoti atsarginį kodą",
    "Cancel login" : "Atsisakyti prisijungimo",
    "Enhanced security is enforced for your account. Choose which provider to set up:" : "Jūsų paskyrai taikomas sustiprintas saugumas. Pasirinkite, kurį teikėją nustatyti:",
    "Error while validating your second factor" : "Klaida patvirtinant jūsų antrąjį faktorių",
    "Access through untrusted domain" : "Prieiga iš nepatikimo domeno",
    "Please contact your administrator. If you are an administrator, edit the \"trusted_domains\" setting in config/config.php like the example in config.sample.php." : "Susisiekite su savo administratoriumi. Jeigu jūs esate administratorius, pataisykite \"trusted_domains\" nustatymą config/config.php faile, kaip tai parodyta pavyzdyje, config.sample.php faile.",
    "Further information how to configure this can be found in the %1$sdocumentation%2$s." : "Tolimesnę informaciją apie tai, kaip tai konfigūruoti, galite rasti %1$sdokumentacijoje%2$s.",
    "Maintenance mode" : "Techninės priežiūros veiksena",
    "This %s instance is currently in maintenance mode, which may take a while." : "Šis %s egzempliorius šiuo metu yra techninės priežiūros veiksenoje, kas savo ruožtu gali šiek tiek užtrukti.",
    "This page will refresh itself when the instance is available again." : "Šis puslapis bus įkeltas iš naujo, kai egzempliorius bus ir vėl prieinamas.",
    "Contact your system administrator if this message persists or appeared unexpectedly." : "Susisiekite su savo sistemos administratoriumi, jei šis pranešimas nedingsta arba, jei jis pasirodė netikėtai.",
    "unknown text" : "nežinomas tekstas",
    "Hello world!" : "Sveikas, pasauli!",
    "sunny" : "saulėta",
    "Hello {name}, the weather is {weather}" : "Sveiki {name}, šiandien yra {weather}",
    "Hello {name}" : "Sveiki, {name},",
    "<strong>These are your search results<script>alert(1)</script></strong>" : "<strong>Paieškos rezultatai:<script>alert(1)</script></strong>",
    "new" : "naujas",
    "_download %n file_::_download %n files_" : ["atsisiųsti %n failą","atsisiųsti %n failus","atsisiųsti %n failų","atsisiųsti %n failą"],
    "An error occurred." : "Įvyko klaida.",
    "Please reload the page." : "Prašome įkelti puslapį iš naujo.",
    "The update was unsuccessful. For more information <a href=\"{url}\">check our forum post</a> covering this issue." : "Atnaujinimas buvo nesėkmingas. Išsamesnei informacijai apie šią problemą, <a href=\"{url}\">žiūrėkite įrašą mūsų forume</a>.",
    "The update was unsuccessful. Please report this issue to the <a href=\"https://github.com/nextcloud/server/issues\" target=\"_blank\">Nextcloud community</a>." : "Atnaujinimas buvo nesėkmingas. Prašome pranešti apie šią problemą <a href=\"https://github.com/nextcloud/server/issues\" target=\"_blank\">„Nextcloud“ bendruomenei</a>.",
    "_The update was successful. Redirecting you to {productName} in %n second._::_The update was successful. Redirecting you to {productName} in %n seconds._" : ["Atnaujinimas sėkmingas. Po %n sekundės būsite peradresuoti į {productName}.","Atnaujinimas sėkmingas. Po %n sekundžių būsite peradresuoti į {productName}.","Atnaujinimas sėkmingas. Po %n sekundžių būsite peradresuoti į {productName}.","Atnaujinimas sėkmingas. Po %n sekundės būsite peradresuoti į {productName}."],
    "Applications menu" : "Programų meniu",
    "Search in current app" : "Ieškoti esamoje programėlėje",
    "Search everywhere" : "Ieškoti visur",
    "Searching …" : "Ieškoma…",
    "Minimum search length is {minSearchLength} characters" : "Minimalus paieškos simbolių skaičius yra {minSearchLength}",
    "Unified search" : "Vieninga paieška",
    "Search apps, files, tags, messages" : "Programėlių, failų, žymų, žinučių paieška",
    "Places" : "Vietos",
    "Filter in current view" : "Filtras dabartiniame rodinyje",
    "Search connected services" : "Ieškoti prijungtų paslaugų",
    "Logging in …" : "Prisijungiama …",
    "We have detected multiple invalid login attempts from your IP. Therefore your next login is throttled up to 30 seconds." : "Dėl daugkartinių nesėkmingų bandymų prisijungti iš jūsų IP adreso, prisijungimai bus uždelsti iki 30 sekundžių.",
    "Loading apps …" : "Įkeliamos programėlės…",
    "Installing apps …" : "Įdiegiamos programėlės…",
    "Loading your contacts …" : "Įkeliami adresatai…",
    "Looking for {term} …" : "Ieškoma {term}…",
    "Search contacts …" : "Ieškoti adresatų…",
    "Loading more results …" : "Įkeliama daugiau rezultatų…",
    "Installing …" : "Įdiegiama…",
    "Search {types} …" : "Ieškoti {types}…",
    "read-only" : "tik skaitymui",
    "_{count} file conflict_::_{count} file conflicts_" : ["{count} failų konfliktas","{count} failų konfliktai","{count} failų konfliktų","{count} failų konfliktas"],
    "One file conflict" : "Vieno failo konfliktas",
    "New Files" : "Nauji failai",
    "Already existing files" : "Jau esami failai",
    "Which files do you want to keep?" : "Kuriuos failus norite pasilikti?",
    "If you select both versions, the copied file will have a number added to its name." : "Jei pasiliekate abi versijas, prie nukopijuoto failo pavadinimo bus pridėtas skaičius.",
    "Cancel" : "Atsisakyti",
    "Continue" : "Tęsti",
    "(all selected)" : "(visi pažymėti)",
    "({count} selected)" : "({count} pažymėtų)",
    "Error loading file exists template" : "Klaida įkeliant saugykloje esančių rinkmenų ruošinį",
    "Saving …" : "Įrašoma…",
    "Connection to server lost" : "Ryšys su serveriu nutrūko",
    "_Problem loading page, reloading in %n second_::_Problem loading page, reloading in %n seconds_" : ["Problemos, įkeliant puslapį. Įkeliama iš naujo po %n sekundės","Problemos, įkeliant puslapį. Įkeliama iš naujo po %n sekundžių","Problemos, įkeliant puslapį. Įkeliama iš naujo po %n sekundžių","Problemos, įkeliant puslapį. Įkeliama iš naujo po %n sekundės"],
    "New in" : "Nauja",
    "View changelog" : "Rodyti keitinių žurnalą",
    "No action available" : "Jokie veiksmai negalimi",
    "Error fetching contact actions" : "Klaida gaunant veiksmus darbui su adresatais",
    "Close \"{dialogTitle}\" dialog" : "Užverti „{dialogTitle}“ dialogą",
    "Non-existing tag #{tag}" : "Neegzistuojanti žyma #{tag}",
    "Restricted" : "Apribota",
    "Delete" : "Ištrinti",
    "Rename" : "Pervadinti",
    "Collaborative tags" : "Bendradarbiavimo žymos",
    "No tags found" : "Nerasta jokių žymų",
    "Please log in before granting %1$s access to your %2$s account." : "Prieš suteikdami %1$s prieigą prie savo %2$s paskyros, prisijunkite.",
    "Currently logged in as %1$s (%2$s)." : "Šiuo metu prisijungta kaip %1$s (%2$s).",
    "You are about to grant %1$s access to your %2$s account." : "Jūs ketinate suteikti %1$s prieigą prie savo %2$s paskyros.",
    "Password sent!" : "Slaptažodis išsiųstas!",
    "You are not authorized to request a password for this share" : "Jūs neturite įgaliojimų prašyti šio bendrinimo slaptažodžio",
    "%1$s will be updated to version %2$s" : "%1$s bus atnaujinta į versiją %2$s",
    "The following apps will be updated:" : "Bus atnaujintos šios programėlės:",
    "The theme %s has been disabled." : "Apipavidalinimas %s buvo išjungtas.",
    "Detailed logs" : "Išsamūs žurnalai",
    "For help, see the  <a target=\"_blank\" rel=\"noreferrer noopener\" href=\"%s\">documentation</a>." : "Prireikus pagalbos, žiūrėkite <a target=\"_blank\" rel=\"noreferrer noopener\" href=\"%s\">dokumentaciją</a>.",
    "Remove filter: {text}" : "Pašalinti filtrą: {text}",
    "Too many incorrect login attempts. Please try again in 30 seconds." : "Per daug neteisingų prisijungimo bandymų. Bandykite dar kartą po 30 sekundžių.",
    "Groups" : "Grupės",
    "Group list is empty" : "Grupės sąrašas yra tuščias",
    "Unable to retrieve the group list" : "Nepavyko atgauti grupių sąrašo",
    "Search apps, files, tags, messages …" : "Ieškokite programų, failų, žymų, pranešimų …"
},
"nplurals=4; plural=(n % 10 == 1 && (n % 100 > 19 || n % 100 < 11) ? 0 : (n % 10 >= 2 && n % 10 <=9) && (n % 100 > 19 || n % 100 < 11) ? 1 : n % 1 != 0 ? 2: 3);");
