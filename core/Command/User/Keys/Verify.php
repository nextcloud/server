...
class Verify extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if ($publicKey != $publicKeyDerived) {
			...
			if ($this->keyManager->setPublicKey($user, $publicKeyDerived)) {
				$output->writeln('<info>Derived public key successfully updated</info>');
				return static::SUCCESS;
			}

			$output->writeln('<error>Updating the public key failed</error>');
			return static::SUCCESS;
		}
