...
import Plugins from './oc-plugins';
import {set, get} from './oc-get-set';
import Util from './oc-util';

window.oc_debug;
window.oc_webroot;

window.oc_current_user = document.getElementsByTagName('head')[0].getAttribute('data-user');
window.oc_requesttoken = document.getElementsByTagName('head')[0].getAttribute('data-requesttoken');

window.oc_config = window.oc_config || {};

if (typeof window.oc_webroot === "undefined") {
	window.oc_webroot = location.pathname;
	var pos = oc_webroot.indexOf('/index.php/');
	if (pos !== -1) {
		window.oc_webroot = oc_webroot.substr(0, pos);
	} else {
		window.oc_webroot = oc_webroot.substr(0, oc_webroot.lastIndexOf('/'));
	}
}
