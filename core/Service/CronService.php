...
class CronService {
	...
	/**
	 * @param callable(string $log) $callback
	 */
	...
	private function runCli(string $appMode, ?array $jobClasses): void {
		...
		while ($job = $this->jobList->getNext($onlyTimeSensitive, $jobClasses)) {
			...
			if ($this->verboseCallback !== null) {
				call_user_func($this->verboseCallback, 'Starting job ' . $jobDetails);
			}
			...
			if ($this->verboseCallback !== null) {
				call_user_func($this->verboseCallback, 'Job ' . $jobDetails . ' done in ' . ($timeAfter - $timeBefore) . ' seconds');
			}
